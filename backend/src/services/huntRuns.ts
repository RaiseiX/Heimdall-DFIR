...
export async function timelineChangedSince(pool: Pool, caseId: string, huntRunId: string): Promise<boolean> {
  const r = await pool.query(
    `SELECT EXISTS (
       SELECT 1 FROM parser_results pr
         JOIN hunt_runs h ON h.id = $2 AND h.case_id = $1
        WHERE pr.case_id = $1 AND pr.parser_name = 'UnifiedTimeline' AND pr.updated_at > h.started_at
     ) AS changed`,
    [caseId, huntRunId]);
  return r.rows[0]?.changed === true;
}
