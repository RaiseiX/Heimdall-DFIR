  ...
  for (const r of rules) {
    ...
    try {
      out.push({
        ...
        types: Array.isArray(r.types) && r.types.length ? new Set(r.types.map(String)) : null,
...
function matchTags(record, descriptionFallback = '', artifactType = '') {
  ...
  const type = String(artifactType || record?.artifact_type || '');
  const hay = {};
  const scanAll = () => {
  ...
  for (const r of rules) {
    if (r.types && !r.types.has(type)) continue;
    let subject;
    if (r.fields) {
