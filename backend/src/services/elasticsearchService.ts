...
export async function deleteByResultId(caseId: string, resultId: string, options: { strict?: boolean } = {}): Promise<void> {
  try {
    ...
    if (options.strict) {
      ...
      const failures = Array.isArray((result as any)?.failures) ? (result as any).failures : [];
      const total = Number((result as any)?.total ?? 0);
      const deleted = Number((result as any)?.deleted ?? 0);
      const incomplete = (result as any)?.timed_out === true
        || failures.length > 0
        || Number((result as any)?.version_conflicts ?? 0) > 0
        || deleted < total;
      if (incomplete) {
        throw Object.assign(new Error('Elasticsearch deletion incomplete'), {
...
export async function bulkIndex(
  ...
  resultId?:  string,
  evidenceId?: string,
): Promise<{ doublons: number; echecs: number }> {
  if (!records.length) return { doublons: 0, echecs: 0 };
  ...
  for (const rec of records) {
    operations.push(rec.dedupe_hash
      ? { create: { _index: index, _id: rec.dedupe_hash } }
      : { index: { _index: index } });
    operations.push({
      case_id:       caseId,
  ...
  if (!response.errors) return { doublons: 0, echecs: 0 };

  const resultats = (response.items as any[]).map(i => i.create ?? i.index ?? {});
  const doublons = resultats.filter(r => r.status === 409).length;
  const echecs = resultats.filter(r => r.error && r.status !== 409);
  if (echecs.length) {
    logger.warn(
      `[ES] bulkIndex partial error (${caseId}): ${echecs.length} item(s), ${JSON.stringify(echecs[0].error).substring(0, 200)}`,
    );
  }
  return { doublons, echecs: echecs.length };
}
