...
// journalctl saute un fichier qu'il ne sait pas lire, le dit sur stderr, et SORT
// ZERO en rendant les entrees des fichiers restants. Une lecture partielle est
// donc indiscernable d'une lecture complete pour qui ne regarde que le code de
// sortie — sept journaux lus sur dix passent pour dix.
//
// Le cas n'est pas theorique : journalctl est en systemd 252 dans l'image
// (Debian bookworm), et un hote collecte en systemd plus recent ecrit des
// journaux que 252 refuse fichier par fichier. Sur une timeline d'incident, des
// segments manquants sans aucun signal valent une conclusion fausse.
const JOURNAL_SKIPPED = [
  /uses an unsupported feature, ignoring file/i,
  /is truncated, ignoring file/i,
  /is corrupted, ignoring file/i,
  /ignoring file/i,
  /Failed to open journal file/i,
  /Cannot read journal file/i,
];

/**
 * Rend une raison quand stderr montre que des fichiers ont ete sautes, sinon
 * null. Le message porte le nombre ET un fichier nomme : « 3 fichiers ignores »
 * n'aide pas a savoir lesquels rechercher.
 */
export function journalIncompleteRead(stderr: string): string | null {
  const lignes = (stderr ?? '').split('\n').map(l => l.trim()).filter(Boolean);
  const sautes = lignes.filter(l => JOURNAL_SKIPPED.some(re => re.test(l)));
  if (!sautes.length) return null;

  const nomme = /([^\s/]+\.journal~?)/.exec(sautes[0])?.[1];
  const exemple = nomme ? ` (par exemple ${nomme})` : '';
  return `journalctl a ignore ${sautes.length} fichier(s) de journal${exemple}`
    + ` : lecture partielle, la timeline est incomplete`;
}
