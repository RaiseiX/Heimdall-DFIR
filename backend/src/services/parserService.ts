...
import { prefetchCommand, PREFETCH_SCRIPT } from './prefetchCommand';
...
export async function runParser(
  ...
  if (parser === 'hayabusa') {
    ...
    const [binary, ...args] = prefetchCommand(inputFile, outputDir, 'output.csv');
...
> {
  ...
  for (const [key, tool] of Object.entries(ZIMMERMAN_TOOLS)) {
    if (key === 'hayabusa') {
      ...
      const scriptPath = PREFETCH_SCRIPT;
