...
import { journalIncompleteRead, journalRows, JOURNAL_FILE_RE } from './catscaleJournal';
...
async function parseJournal(
  ...
  // Une lecture PARTIELLE se signale aussi. journalctl saute un fichier illisible,
  // le dit sur stderr, et sort zero en rendant le reste : stderr n'etait consulte
  // que dans la branche « zero ligne », donc sept journaux lus sur dix passaient
  // pour dix. Le critere est « un parseur en erreur ne produit jamais un succes
  // vide » — un succes partiel doit se voir tout autant.
  const partiel = journalIncompleteRead(stderr);
  if (partiel) {
    failures.push({ stage: 'parse', target: journalDir, reason: partiel });
  }
  if (spawnError) {
