'use strict';
const { libellesNarratif } = require('./reportNarrative');
...
function drawCover(doc, d, A) {
  ...
  const badge = d.aiNarrative ? libellesNarratif(d.narrativeOrigine).badge : null;
  if (badge) {
    doc.rect(MX, PH - 24, 5, 5).fill(A);
    doc.fillColor(A).font(FB).fontSize(7).text(badge, MX + 9, PH - 26, { characterSpacing: 0.6, lineBreak: false });
  }
}
...
function drawExecutive(doc, d, A) {
  ...
  if (d.aiNarrative?.executive_summary) callout(doc, libellesNarratif(d.narrativeOrigine).synthese, d.aiNarrative.executive_summary, A);
}
...
function drawAiAnalysis(doc, d, num, A) {
  ...
  doc.addPage(); doc.y = TOP;
  const libelles = libellesNarratif(d.narrativeOrigine);
  sectionTitle(doc, num, libelles.titre, A);
  para(doc, libelles.intro, { color: FAINT, size: 8.5 });
  doc.y += 6;
...
const STATUT_SOURCE = {
  resolue: null,
  supprimee: 'preuve supprimée depuis la rédaction',
  introuvable: 'preuve introuvable dans ce dossier',
  hors_dossier: 'source hors dossier, non résolue',
};

function drawSources(doc, d, num, A) {
  const sources = d.sources || [];
  if (!sources.length) return false;
  doc.addPage(); doc.y = TOP;
  sectionTitle(doc, num, 'Sources', A);
  para(doc, "Chaque renvoi [n] du rapport renvoie à la ligne correspondante. Les empreintes sont celles enregistrées à l'import de la preuve.", { color: FAINT, size: 8.5 });
  doc.y += 4;
  for (const s of sources) {
    ensure(doc, 44);
    const statut = STATUT_SOURCE[s.statut];
    doc.fillColor(INK).font(FB).fontSize(9.5).text(`[${s.n}]  ${s.preuve || statut || ''}`, MX, doc.y, { width: CW });
    if (s.preuve && statut) para(doc, statut, { color: SUB, size: 8.5, gap: 2 });
    if (s.sha256) doc.fillColor(SUB).font(FM).fontSize(7.5).text(`SHA-256 ${s.sha256}`, MX + 18, doc.y + 1, { width: CW - 18 });
    if (s.element) para(doc, s.element, { color: SUB, size: 8.5, x: MX + 18, width: CW - 18, gap: 2 });
    doc.y += 6;
  }
  return true;
}
...
function renderReport(doc, d) {
  ...
  if (d.aiNarrative?.recommendations) {
    ...
    callout(doc, libellesNarratif(d.narrativeOrigine).remediation, d.aiNarrative.recommendations, A);
    n++;
  }

  drawSources(doc, d, n, A);
