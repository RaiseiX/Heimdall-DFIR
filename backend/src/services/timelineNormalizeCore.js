...
const INVENTORY_TS_KIND = 'inventory';

function extractTimestampOrInventory(record, timestampColumns) {
  const columns = Array.isArray(timestampColumns) ? timestampColumns : [];
  return extractTimestamp(record, columns) || { timestamp: null, column: INVENTORY_TS_KIND };
}
...
function empreinteDuContenu(record) {
  const r = record || {};
  return crypto.createHash('md5').update(JSON.stringify(Object.keys(r).sort().map(k => [k, r[k]]))).digest('hex');
}
...
module.exports = { stripNullBytes, normalizeTimestamp, extractTimestamp, extractTimestampOrInventory, INVENTORY_TS_KIND, extractDescription, computeDedupeHash, TIMELINE_FIELD_CONFIG };
