...
const { stripNullBytes, extractTimestampOrInventory, extractDescription, computeDedupeHash, TIMELINE_FIELD_CONFIG } = require('./timelineNormalizeCore');
...
async function importCsvToTimeline(csvPath, { pool, caseId, resultId, evidenceId, artifactType }) {
  ...
  const flush = async () => {
    ...
    const res = await pool.query(
      `INSERT INTO collection_timeline
         (case_id, result_id, evidence_id, timestamp, timestamp_kind, artifact_type, artifact_name, description, source, host_name, raw, dedupe_hash)
       SELECT $1,$2,$3, u.ts, u.kind, $4, $5, u.descr, u.src, u.hn, u.rw, u.dh
         FROM UNNEST($6::timestamptz[], $7::text[], $8::text[], $9::text[], $10::jsonb[], $11::text[], $12::text[])
              AS u(ts, descr, src, hn, rw, dh, kind)
  ...
  for await (const line of rl) {
    ...
    const ts = extractTimestampOrInventory(record, cfg.timestampColumns);
    ...
    batch.push({ ts: ts.timestamp, kind: ts.column, descr: description || null, src: source, hn: host, raw, dh });
