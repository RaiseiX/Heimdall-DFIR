...
import { reconcileStaleHunts, timelineChangedSince } from '../services/huntRuns';
const { runAllEngines, triggerHunt } = require('../services/runAllService');

export type HuntDeps = {
  runAllEngines: (pool: Pool, caseId: string, userId: string, huntRunId: string) => Promise<void>;
  pool: Pool;
  timelineChangedSince?: (pool: Pool, caseId: string, huntRunId: string) => Promise<boolean>;
  retrigger?: (pool: Pool, caseId: string, userId: string) => Promise<unknown>;
};
...
export async function processHunt(data: HuntJobData, deps: HuntDeps): Promise<void> {
  ...
  if (deps.timelineChangedSince && deps.retrigger
      && await deps.timelineChangedSince(deps.pool, data.caseId, data.huntRunId)) {
    await deps.retrigger(deps.pool, data.caseId, data.userId);
  }
}
...
if (process.env.NODE_ENV !== 'test') {
  ...
  new Worker<HuntJobData>('hunting-jobs', async (job: Job<HuntJobData>) => {
    await processHunt(job.data, {
      runAllEngines, pool, timelineChangedSince,
      retrigger: (p: Pool, caseId: string, userId: string) => triggerHunt(p, caseId, userId, 'auto'),
    });
