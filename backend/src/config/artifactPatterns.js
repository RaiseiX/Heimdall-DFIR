...
const { PREFETCH_SCRIPT } = require('../services/prefetchCommand');
const fs = require('fs');
...
const ARTIFACT_PATTERNS = {
  ...
  prefetch: {
    ...
    toolKey: 'prefetch',
    name: 'PECmd',
    // PECmd ne tourne pas sous Linux : il sort ZERO en refusant, donc l'appelant
    // lit « parse reussi, zero prefetch ». Ce builder est aujourd'hui injoignable
    // — `prefetch` est la premiere branche de la chaine de routes/collection.js —
    // mais il redeviendrait vivant si cette chaine etait reordonnee. Le laisser
    // armé reviendrait à poser un piège pour plus tard.
    //
    // Le litteral `'--csvf', 'prefetch_results.csv'` doit rester ecrit tel quel :
    // deriveMappings.js lit la SOURCE de ce builder pour en extraire le nom de
    // fichier, et un variable a la place rendrait null en silence.
    argsBuilder: (input, output) => {
      const dir = fs.statSync(input).isDirectory() ? input : path.dirname(input);
      return ['python3', PREFETCH_SCRIPT, '-d', dir, '--csv', output, '--csvf', 'prefetch_results.csv'];
    },
