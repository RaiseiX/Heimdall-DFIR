...
const { authGraphSql, construireGraphe, EVENEMENTS_WINDOWS, SOURCES_LINUX } = require('../services/authGraph');
...
const UUID_EVIDENCE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

router.get('/:caseId/auth-graph', authenticate, requireRole('admin', 'analyst'), async (req, res) => {
  const { caseId } = req.params;
  const evidenceId = req.query.evidence_id;
  if (typeof evidenceId !== 'string' || !UUID_EVIDENCE.test(evidenceId)) {
  ...
  try {
    ...
    const { rows } = await readPool.query(authGraphSql(), [caseId, evidenceId, EVENEMENTS_WINDOWS, SOURCES_LINUX]);
    res.json({ evenementsWindows: EVENEMENTS_WINDOWS, ...construireGraphe(rows, { inclureServices: req.query.services === '1' }) });
  } catch (err) {
