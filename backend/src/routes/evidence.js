...
const { deleteByResultId } = require('../services/elasticsearchService');
const { planEvidenceDeletion } = require('../services/evidenceDeletionPlan');
const { racinesDeStockage, estSousUneRacine } = require('../services/storagePathGuard');
const {
  withCaseDeletion,
...
const { lignesParCollecteSql } = require('../services/evidenceRowCounts');
...
router.get('/:caseId', authenticate, async (req, res) => {
  try {
    const [result, comptes] = await Promise.all([
      pool.query(`
      ...
      `, [req.params.caseId]),
      pool.query(lignesParCollecteSql(), [req.params.caseId]),
    ]);

    const parCollecte = new Map(comptes.rows.map(r => [r.evidence_id, Number(r.lignes)]));
    res.json(result.rows.map(e => ({ ...e, timeline_rows: parCollecte.get(e.id) ?? 0 })));
  } catch (err) {
...
router.delete('/:id', authenticate, async (req, res) => {
  ...
  try {
    ...
    const identite = await withCaseDeletion(pool, caseId, async client => {
      const row = await client.query(
        'SELECT file_path, name, additional_files, original_filename, file_size, hash_md5, hash_sha1, hash_sha256 FROM evidence WHERE id = $1 AND case_id = $2 FOR UPDATE',
      ...
      const parserResults = await client.query(
        'SELECT id FROM parser_results WHERE evidence_id = $1 AND case_id = $2 ORDER BY id FOR UPDATE',
        [req.params.id, caseId]
      );
      const resultIds = parserResults.rows.map(result => String(result.id));
      let targets;
      try {
      ...
      const operation = await beginDeletionOperation(pool, {
        ...
        items: [
          ...targets,
          ...resultIds.map(resultId => ({ kind: 'elasticsearch', locator: resultId })),
        ],
      });
      ...
      for (const item of operation.items) {
        ...
        try {
          ...
          } else if (item.kind === 'elasticsearch') {
            await deleteByResultId(caseId, item.locator, { strict: true });
            result = { status: 'deleted', method: 'delete_by_result_id' };
          } else {
            const resolved = path.resolve(item.locator);
            const racine = estSousUneRacine(racinesDeStockage(), resolved);
            if (!racine) {
              throw Object.assign(new Error('Storage target outside configured roots'), { code: 'STORAGE_PATH_REJECTED' });
            }
            result = await destroyStorageTarget(resolved, racine);
          }
          ...
          throw Object.assign(new Error('Suppression incomplète'), { status: 502, code: 'DELETION_INCOMPLETE', cible: { type: item.kind, code } });
        }
      }
      ...
      return {
        ...
        file_size: evidence.file_size == null ? null : Number(evidence.file_size),
        sha256: evidence.hash_sha256 ?? null,
        sha1: evidence.hash_sha1 ?? null,
        md5: evidence.hash_md5 ?? null,
      };
    });

    await auditLog(req.user.id, 'delete_evidence', 'evidence', req.params.id, identite, req.ip);
    ...
    if (err.code === 'DELETION_INCOMPLETE') return res.status(502).json({ error: 'Suppression incomplète. Réessayez ultérieurement.', code: err.code, operation_id: operationId, ...(err.cible ? { cible: err.cible } : {}) });
