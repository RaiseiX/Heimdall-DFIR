...
const { prefetchCommand } = require('../services/prefetchCommand');
...
const { compteWindows } = require('../services/windowsAccount');
const { windowsProcessEventsSql } = require('../services/windowsProcessEvents');
const {
  binaireAmcacheSql, binaireMftSql, executionPrefetchSql, executionSrumSql,
  homonymesSql, repartitionEvenementsSql, cheminRelatif,
} = require('../services/windowsProcessDossier');
...
const { destroyStorageTarget } = require('../services/storageDeletionService');
...
const { pushTimelineFilters, aDesFiltresAvances, validerEvidenceIds } = require('../services/timelineFilters');
...
const { stripNullBytes, normalizeTimestamp, extractTimestamp, extractTimestampOrInventory, extractDescription } = require('../services/timelineNormalizeCore');
...
const { cheminCorrespond } = require('../services/artifactMatch');
const { perimetreDeParsing } = require('../services/parseScope');
const { arbreDeCollecte } = require('../services/collectionTree');
const { mesurerLecture, parseursInitiaux } = require('../services/parseWeight');
const { creerFileParCas } = require('../services/caseQueue');
const fileDurable = require('../services/parseQueue');
const { executerDansJournal, lireJournal } = require('../services/journalParsing');
...
const { reprendreLnkNonUtf8 } = require('../services/nomsNonUtf8');
const { commonParserDir } = require('../services/commonParserDir');
const { resoudreRacineDeCollecte } = require('../services/collectionRoot');
const { racinesDeStockage } = require('../services/storagePathGuard');
const { assainirOptions, drapeauxDe } = require('../config/parserOptions');
const { resumeArtefactsSql, resumeArtefacts } = require('../services/artifactSummary');
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (isDir) {
          ...
          const relatif = path.relative(dir, fullPath);
          for (const pattern of patterns) {
            if (cheminCorrespond(relatif, pattern)) {
...
const fileDeParsing = creerFileParCas();
...
function updateParseProgress(caseId, validTypes, data) {
  ...
  if (data.type === 'start' || !e) {
    e = {
      parsers: parseursInitiaux(validTypes, data.poids, k => ARTIFACT_PATTERNS[k]?.name || k),
      globalPct: 0,
      plateforme: data.plateforme ?? null,
      masques: Array.isArray(data.masques) ? data.masques : [],
      raison: data.raison ?? null,
      octetsDistincts: Number.isInteger(data.octetsDistincts) ? data.octetsDistincts : null,
      collection_dir: data.collection_dir ?? null,
      updatedAt: Date.now(),
    };
...
async function streamNormalizeToDB(csvPath, caseId, resultId, artifactType, config, evidenceId = null, sourceDevice = null) {
  ...
  return new Promise((resolve, reject) => {
    ...
    csvParser.on('data', (rawRecord) => {
      ...
      const tsResult = extractTimestampOrInventory(clean, config.timestampColumns);
...
const { ARTIFACT_TYPE_PLATFORM, platformForArtifactType } = require('../services/artifactPlatform');
...
router.post('/:caseId/import', authenticate, upload.single('collection'), async (req, res) => {
  ...
  try {
    ...
    executerDansJournal({ caseId, preuve: collectionDir, emettre: ligne => { if (io) io.to(`case:${caseId}`).emit('collection:log', { ...ligne, caseId }); } }, async () => {
      try {

        let fileHashes = { md5: null, sha1: null, sha256: null };
        let archiveSize = null;
        try { archiveSize = fs.statSync(uploadedPath).size; } catch (_e) {}
        try {
        ...
        await pool.query(
          ...
          [caseId, 'Collecte: ' + originalFilename, originalFilename, collectionDir,
           ...
           userId, JSON.stringify({ detected: detectedArtifacts, total_files: totalFiles, archive_size: archiveSize, archive_name: originalFilename }),
        ...
        if (socketId && io) {
          io.to(socketId).emit('collection:import:done', {
            ...
            arbre: arbreDeCollecte(detectedArtifacts, collectionDir),
...
router.get('/:caseId/parse-log', authenticate, (req, res) => {
  const depuis = Number.parseInt(req.query.depuis, 10);
  res.json({ lignes: lireJournal(req.params.caseId, Number.isFinite(depuis) && depuis > 0 ? depuis : 0) });
});

router.get('/:caseId/parse-progress', authenticate, async (req, res) => {
  const { caseId } = req.params;
  const e = PARSE_PROGRESS.get(caseId);
  if (!e) return res.json({ active: false, live: false, globalPct: 0, parsers: {}, plateforme: null, masques: [] });
  ...
  if (e.done || e.globalPct >= 100 || age > 5 * 60 * 1000) {
    ...
    return res.json({ active: false, live: false, globalPct: 0, parsers: {}, plateforme: null, masques: [] });
  }
  res.json({ active: true, live: age < 15000, globalPct: e.globalPct, parsers: e.parsers, plateforme: e.plateforme ?? null, masques: e.masques || [], raison: e.raison ?? null, octetsDistincts: e.octetsDistincts ?? null, collection_dir: e.collection_dir ?? null });
});
...
function lancerParsing(ctx, idFile) {
  return fileDeParsing.enchainer(ctx.caseId, async () => {
    try { await fileDurable.demarrer(pool, idFile); } catch (e) { logger.warn(`[parse-queue] demarrage ${idFile} : ${e.message}`); }
    const emettre = ligne => { if (ctx.io) ctx.io.to(`case:${ctx.caseId}`).emit('collection:log', { ...ligne, caseId: ctx.caseId }); };
    const issue = await executerDansJournal({ caseId: ctx.caseId, preuve: ctx.collDir, emettre }, () => executerParsing(ctx));
    try { await fileDurable.terminer(pool, idFile, issue.statut, issue.erreur); } catch (e) { logger.warn(`[parse-queue] cloture ${idFile} : ${e.message}`); }
  }).catch(() => {});
}

function reprendreFile(io) {
  return fileDurable.reprendreParsings({
    aReprendre: () => fileDurable.aReprendre(pool),
    terminer: (id, statut, erreur) => fileDurable.terminer(pool, id, statut, erreur),
    purger: ligne => withCaseDeletion(pool, ligne.case_id, async dbClient => {
      await dbClient.query('DELETE FROM collection_timeline WHERE case_id = $1 AND result_id = $2', [ligne.case_id, ligne.result_id]);
      await esService.deleteByResultId(ligne.case_id, ligne.result_id, { strict: true });
    }),
    lancer: ligne => lancerParsing({ ...fileDurable.contexteDeReprise(ligne), io }, ligne.id),
    relancerDetection: ligne => {
      const { startRunAll } = require('../services/runAllService');
      Promise.resolve(startRunAll(ligne.case_id, { id: ligne.payload.userId }, 'auto'))
        .catch(e => logger.warn(`[parse-queue] detection ${ligne.id} : ${e.message}`));
    },
    existe: chemin => fs.existsSync(chemin),
    logger,
  });
}

async function executerParsing({ caseId, resultId, collDir, perimetre, typesToParse, validTypes, lecture, evidenceId, sourceDevice, userId, ip, exhaustiveFsTimeline, parserOptions, socketId, io }) {
  const totalTypes = validTypes.length;
  let currentType = 0;
  const results = {};
  let totalRecords = 0;
  let issue = { statut: 'done' };
  ...
  function emitProgress(data) {
    ...
    try { updateParseProgress(caseId, validTypes, data); } catch (_e) {}
      ...
      const debutParsing = new Date().toISOString();
      emitProgress({ type: 'start', collection_dir: collDir, total: totalTypes, artifacts: validTypes, poids: lecture.poids, octetsDistincts: lecture.distincts, plateforme: perimetre.plateforme, masques: perimetre.masques.map(cle => ({ cle, plateforme: platformForArtifactType(cle) })), raison: perimetre.raison });
      try {
  ...
  await runConcurrent(typesToParse, async (artifactType) => {
    ...
    try {
      ...
      if (artifactType === 'prefetch') {
        ...
        toolArgs = prefetchCommand(pfDir, outputDir, 'prefetch_results.csv');
        ...
        if (artifactType === 'lnk') lnkNonUtf8 = dirInput;
      ...
      if (toolArgs !== null) toolArgs = [...toolArgs, ...drapeauxDe(artifactType, parserOptions)];
      ...
      if (lnkNonUtf8) {
        try {
          const { repris } = await reprendreLnkNonUtf8({ dossier: lnkNonUtf8, sortie: outputDir, zimmermanDir: ZIMMERMAN_DIR,
            lancerOutil: args => spawnTool(args, { timeout: 600000, maxBuffer: 1024 * 1024 * 64, cwd: outputDir }) });
          if (repris.length) logger.info(`[parse] lnk: ${repris.length} fichier(s) au nom non UTF-8 relu(s) par alias : ${repris.join(' | ')}`);
        } catch (e) {
          logger.warn(`[parse] lnk: reprise des noms non UTF-8 impossible : ${e.message}`);
        }
      }
  ...
  try {
    await pool.query(
      `UPDATE parser_results
       ...
       WHERE id = $3`,
      [
        JSON.stringify({ parse_results: results, artifact_types: typesToParse, total_records: totalRecords, parser_options: parserOptions || null }),
        totalRecords,
        resultId,
      ]
    );
    ...
    await auditLog(userId, 'parse_collection', 'collection', resultId,
      { artifact_types: typesToParse, total_records: totalRecords }, ip);
    ...
    try {
      const { startRunAll } = require('../services/runAllService');
      startRunAll(caseId, { id: userId }, 'auto');
      if (io) io.to(`user:${userId}`).emit('notification:job_done', {
    ...
    issue = { statut: 'error', erreur: dbErr.message };
    if (io) {
      ...
      io.to(`user:${userId}`).emit('notification:job_done', {
        ...
        issue = { statut: 'error', erreur: parseErr.message };
        if (io) {
          if (socketId) io.to(socketId).emit('collection:parse:error', {
            collection_dir: collDir,
...
router.post('/:caseId/parse', authenticate, async (req, res) => {
  ...
  const parserOptions = assainirOptions({
    ...(req.body?.parser_options || {}),
    catscale: {
      exhaustive_fs_timeline: req.body?.exhaustive_fs_timeline,
      ...((req.body?.parser_options && req.body.parser_options.catscale) || {}),
    },
  });
  ...
  try {
    const racine = await resoudreRacineDeCollecte(
      pool, caseId,
      { evidenceId: bodyEvidenceId, collectionDir: collection_dir },
      { racines: [COLLECTIONS_DIR, ...racinesDeStockage()] }
    );
    collDir = racine.chemin;
    evidenceIdFromBody = racine.evidenceId;
  } catch (e) {
    if (e.code === 'INTROUVABLE') {
      return res.status(400).json({ error: 'Repertoire de collecte invalide. Importez une collecte d\'abord.' });
    }
    if (e.status === 400 || e.status === 403) return res.status(e.status).json({ error: e.message, code: e.code });
    logger.error('[parse] resolution de la collecte:', e.message);
    return res.status(500).json({ error: 'Erreur de résolution de la collecte' });
  }
  ...
  try {
    const det = await pool.query(
      `SELECT output_data->'detected' AS detected FROM parser_results
        WHERE case_id = $1 AND parser_name = 'MagnetRESPONSE_Import' AND input_file = $2
        ORDER BY created_at DESC LIMIT 1`, [caseId, collDir]);
    const brut = det.rows[0]?.detected;
    if (brut && typeof brut === 'object') {
      detection = brut;
      detectes = Object.keys(brut);
    }
  } catch (_e) {}

  const perimetre = perimetreDeParsing({
    demandes: requestedTypes,
    detectes,
    catalogue: Object.keys(ARTIFACT_PATTERNS),
    plateformeDe: platformForArtifactType,
  });

  const typesToParse = perimetre.aParser;

  const io       = req.app.locals.io;
  const socketId = req.body.socketId || null;
  const validTypes = typesToParse.filter(t => ARTIFACT_PATTERNS[t]);
  const detectionRetenue = Object.fromEntries(validTypes.filter(k => detection && detection[k]).map(k => [k, detection[k]]));
  const lecture = mesurerLecture(detectionRetenue, (chemin) => fs.statSync(chemin).size);
  ...
  const charge = {
    userId: req.user.id,
    ip: req.ip,
    collDir, evidenceId, sourceDevice, perimetre, typesToParse, validTypes, lecture,
    exhaustiveFsTimeline: parserOptions.catscale.exhaustive_fs_timeline,
    parserOptions,
  };
  ...
  let idFile;
  try {
    ({ id: resultId, idFile } = await withCaseDeletion(pool, caseId, async dbClient => {
      ...
      const id = prRow.rows[0].id;
      return { id, idFile: await fileDurable.inscrire(dbClient, { caseId, resultId: id, payload: charge }) };
    }));
  } catch (initErr) {
  ...
  await esService.ensureIndex(caseId).catch(e =>
    ...
    lancerParsing({ ...charge, caseId, resultId, socketId, io }, idFile);
});
...
router.get('/:caseId/timeline', authenticate, async (req, res) => {
  try {
    ...
    pi = pushTimelineFilters(req.query, {
      conditions, params, pi, hostCol, validatedEvidenceIds, huntPredicate,
    });
...
router.get('/:caseId/timeline/groups', authenticate, async (req, res) => {
  try {
    ...
    const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (evidence_id && !UUID_RE.test(evidence_id)) {
      return res.status(400).json({ error: 'evidence_id invalide' });
    }
    let evidenceIdsGroupes = null;
    if (!evidence_id && evidence_ids) {
      const ids = String(evidence_ids).split(',').map(s => s.trim()).filter(Boolean);
      if (ids.some(id => !UUID_RE.test(id))) {
        return res.status(400).json({ error: 'evidence_ids invalides' });
      }
      if (ids.length) evidenceIdsGroupes = ids;
    }
    ...
    pi = pushTimelineFilters(req.query, {
      conditions, params, pi, hostCol: hostColG,
      validatedEvidenceIds: evidenceIdsGroupes, huntPredicate,
    });
...
router.get('/:caseId/processes', authenticate, async (req, res) => {
  try {
    ...
    if (req.query.with === 'windows') {
      ...
      const avecCompte = w.rows.map(r => {
        const compte = compteWindows(r.user_sid, r.user_name);
        return { ...r, user_name: compte.libelle, user_source: compte.source };
      });
      return res.json({ processes: avecCompte, snapshot: false, source: 'evtx_4688' });
    }
    ...
    if (req.query.with === 'dossier') {
      const pid = Number(req.query.pid);
      const chemin = String(req.query.path || '');
      const depuis = String(req.query.from || '');
      if (!Number.isSafeInteger(pid) || pid <= 0 || !chemin || !Number.isFinite(Date.parse(depuis))) {
        return res.status(400).json({ error: 'pid, path et from requis' });
      }
      const jusqua = req.query.to ? String(req.query.to) : null;
      if (jusqua !== null && !Number.isFinite(Date.parse(jusqua))) {
        return res.status(400).json({ error: 'to invalide' });
      }

      const rel = cheminRelatif(chemin);
      const base = rel.split('\\').pop();
      const [am, mft, pf, sr, ho, rep] = await Promise.all([
        readPool.query(binaireAmcacheSql(), [caseId, evidenceId, rel]),
        readPool.query(binaireMftSql(), [caseId, evidenceId, rel, base]),
        readPool.query(executionPrefetchSql(), [caseId, evidenceId, base]),
        readPool.query(executionSrumSql(), [caseId, evidenceId, rel]),
        readPool.query(homonymesSql(), [caseId, evidenceId, base, rel]),
        readPool.query(repartitionEvenementsSql(), [caseId, evidenceId, String(pid), depuis, jusqua, 12]),
      ]);

      return res.json({
        amcache: am.rows[0] || null,
        mft: mft.rows[0] || null,
        prefetch: pf.rows,
        srum: sr.rows[0] || null,
        homonymes: ho.rows,
        repartition: rep.rows,
      });
    }
    ...
    if (req.query.with === 'events') {
      ...
      if (depuis) {
        if (!Number.isFinite(Date.parse(depuis))) {
          return res.status(400).json({ error: 'from invalide' });
        }
        const jusqua = req.query.to ? String(req.query.to) : null;
        if (jusqua !== null && !Number.isFinite(Date.parse(jusqua))) {
          return res.status(400).json({ error: 'to invalide' });
        }
        const plafond = Math.min(Number(req.query.limit) || 500, 2000);
        const w = await readPool.query(windowsProcessEventsSql(),
          [caseId, evidenceId, String(pid), depuis, jusqua, plafond]);
        return res.json({
          events: w.rows,
          scope: { kind: 'boot_session', taken_at: depuis, until: jusqua },
          capped: w.rows.length >= plafond,
        });
      }
...
router.post('/:caseId/hayabusa', authenticate, async (req, res) => {
  try {
    ...
    async function replaceHayabusaGeneration(outputDataJson, recordCount = 0, writeGeneration = async () => {}) {
      return withCaseDeletion(pool, caseId, async dbClient => {
        ...
        try {
          await writeGeneration(dbClient, newRow.rows[0].id);
        } catch (error) {
          await markDeletionOperation(pool, operation.id, 'incomplete', safeErrorCode(error));
          throw Object.assign(new Error('Hayabusa replacement incomplete'), {
            status: 502,
            code: 'DELETION_INCOMPLETE',
            operationId: operation.id,
    ...
    try {
      ...
      if (fs.existsSync(outputFile) && fs.statSync(outputFile).size > 0) {
          ...
          for (const p of items) {
            ...
            const dedupe = recId
              ...
              : crypto.createHash('md5')
                  ...
                  .digest('hex').slice(0, 16);
            let tags = [];
            try { tags = matchKeywordTags({ level: lvl, description: desc }, desc, 'hayabusa'); } catch (_e) {}
        ...
        await replaceHayabusaGeneration(
          JSON.stringify({ evtx_dir: evtxParentDir, evtx_files_count: evtxFiles.length }),
          0,
          async (dbClient, newId) => {
            streamResultId = newId;
              ...
              rl.on('line', (line) => {
                ...
                if (batch.length >= 2000) {
                  ...
                  pendingInsert = new Promise(r => setImmediate(r))
                    .then(() => insertHayBatch(cur, dbClient))
                    .then(() => rl.resume())
                    .catch(reject);
                }
              });
              rl.on('close', async () => {
                try {
                  ...
                  if (batch.length > 0) {
                    ...
                    await insertHayBatch(batch, dbClient);
                  }
                  resolve();
                } catch (e) { reject(e); }
              });
    ...
    const { newId: hayResultId } = await replaceHayabusaGeneration(
      JSON.stringify({
      ...
      async (dbClient, newId) => {
        if (hayabusaRecords.length === 0) return;
        const CT_BATCH = 500;
        for (let i = 0; i < hayabusaRecords.length; i += CT_BATCH) {
          ...
          const hayDedupe = crypto.createHash('md5')
            ...
            .digest('hex').slice(0, 16);
          let hayTags = [];
          try { hayTags = matchKeywordTags(r, r.description, 'hayabusa'); } catch (_e) {}
  ...
  try {
    ...
    const result = await withCaseDeletion(pool, caseId, async client => {
      const generationRows = await client.query(
        `SELECT id::text AS generation_id, parser_name,
                COALESCE(NULLIF(output_data->>'collection_dir', ''), input_file) AS dir
         FROM parser_results
         WHERE case_id = $1
      ...
      let freedBytes = 0;
      const collDirs = [...new Set(generationRows.rows
        .filter(row => row.parser_name === 'MagnetRESPONSE_Import')
        .map(row => row.dir)
      ...
      const operation = await beginDeletionOperation(pool, {
        ...
        requestIp: req.ip,
        items: [
          ...collDirs.map(locator => ({ kind: 'disk', locator })),
      ...
      for (const item of operation.items.filter(entry => entry.kind === 'disk')) {
        ...
        try {
          ...
          const storageRoot = roots.find(root => resolved.startsWith(root + path.sep));
          ...
          const deletion = await destroyStorageTarget(resolved, storageRoot);
          await markDeletionItem(pool, operation.id, item.item_key, deletion.status, deletion.method);
        } catch (error) {
          const code = safeErrorCode(error);
...
router.get('/:caseId/export/csv', authenticate, async (req, res) => {
  try {
    ...
    const { artifact_types, evidence_id } = req.query;
    ...
    const ev = await validerEvidenceIds(pool, caseId, req.query);
    if (!ev.ok) return res.status(ev.status).json({ error: ev.error });

    const conditions = ['case_id = $1'];
    const params = [caseId];
    let pi = 2;

    pi = pushTimelineFilters(req.query, { conditions, params, pi, validatedEvidenceIds: ev.ids });
...
router.get('/:caseId/export-csv-stream', authenticate, async (req, res) => {
  const { caseId } = req.params;
  const { artifact_types, search, start_time, end_time, result_id, evidence_id, sep = ',' } = req.query;
  ...
  let written = 0;
  try {
    const hasIndex = await esService.indexExists(caseId) && !aDesFiltresAvances(req.query);
    ...
    if (hasIndex) {
      ...
      const params = [caseId];
      let pi = 2;
      pi = pushTimelineFilters(req.query, { conditions, params, pi });
...
router.get('/:caseId/artifact-summary', authenticate, async (req, res) => {
  const { caseId } = req.params;
  const evidenceId = req.query.evidence_id;
  if (typeof evidenceId !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(evidenceId)) {
    return res.status(400).json({ error: 'evidence_id invalide' });
  }
  try {
    const ev = await readPool.query('SELECT 1 FROM evidence WHERE id = $1 AND case_id = $2', [evidenceId, caseId]);
    if (!ev.rowCount) return res.status(404).json({ error: 'Collecte introuvable' });
    const { rows } = await readPool.query(resumeArtefactsSql(), [caseId, evidenceId]);
    res.json(resumeArtefacts(rows));
  } catch (err) {
    logger.error('[collection] GET artifact-summary:', err.message);
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
...
router.delete('/:caseId/verdicts/:eventRef', authenticate, async (req, res) => {
  try {
    const database = req.app.locals.pool;
    await withCaseDeletion(database, req.params.caseId, client => client.query(
      `DELETE FROM artifact_verdicts WHERE case_id = $1 AND event_ref = $2`,
      [req.params.caseId, req.params.eventRef]
    ));
    await auditLog(req.user.id, 'delete_artifact_verdict', 'case', req.params.caseId,
      { event_ref: req.params.eventRef }, req.ip);
    res.json({ ok: true });
  } catch (err) {
    if (err.code === 'LEGAL_HOLD') return res.status(409).json({ error: 'Suppression interdite : legal hold actif.', code: 'LEGAL_HOLD' });
    if (err.status === 404) return res.status(404).json({ error: 'Dossier introuvable' });
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
...
module.exports.streamNormalizeToDB = streamNormalizeToDB;
module.exports.reprendreFile = reprendreFile;
