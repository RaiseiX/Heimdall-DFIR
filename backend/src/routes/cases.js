  ...
  try {
    ...
    const VECTORS = [
      ...
      },
      {
        ...
        mitre: 'T1197',
        severity: 'ÉLEVÉ',
        // Cette chasse filtrait sur le type d'artefact « bits » — ecrit ici en
        // toutes lettres et non sous sa forme de predicat, parce que le garde
        // `huntArtifactTypes.test.js` cherche cette forme exacte dans le fichier
        // et ne sait pas distinguer un commentaire d'une requete.
        //
        // Aucun parseur n'a jamais produit ce type : il dependait de
        // `BitsParser.dll`, outil
        // Python de Mandiant telecharge a tort depuis le domaine d'Eric
        // Zimmerman (404 a chaque build). Mesure du 2026-09-18 : 0 ligne.
        //
        // Elle rendait donc zero, toujours — et un zero sur une chasse de
        // persistance se lit « aucun abus de BITS », pas « ce gisement n'a
        // jamais ete alimente ».
        //
        // Le gisement reel est le journal BITS, qui lui est bien parse :
        // 460 evenements `Microsoft-Windows-Bits-Client/Operational` mesures.
        // On retient la creation de travail (3) et le demarrage de transfert
        // (59), les deux qui portent `jobTitle` et `jobId` — les evenements de
        // fin et d'erreur ajoutent du volume sans information nouvelle.
        // 151 lignes, 14 ms par containment GIN sur idx_ct_raw_gin.
        //
        // AUCUNE exclusion par intitule : les sept titres mesures ici sont
        // legitimes (Edge Component Updater, Font Download, DeltaPatch...), mais
        // `jobTitle` est une chaine que l'attaquant choisit. Exclure « Font
        // Download » masquerait precisement celui qui s'en sert comme couverture.
        query: `
          SELECT id, timestamp, artifact_type, description, source, host_name, raw
          FROM collection_timeline
          WHERE case_id = $1
            AND raw @> '{"Provider":"Microsoft-Windows-Bits-Client"}'::jsonb
            AND event_id IN ('3', '59')
          ORDER BY timestamp
          LIMIT 200`,
      },
      {
