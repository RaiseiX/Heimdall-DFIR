...
const { origineDuNarratif } = require('../services/reportNarrative');
const { numeroterSources, resoudreSources } = require('../services/reportSources');
const notebookDocs = require('../services/notebookDocs');

const lireCarnet = (caseId) => notebookDocs.lire(pool, caseId).then((r) => r.content || '').catch(() => '');
const {
...
router.post('/:caseId/ai-draft', authenticate, async (req, res) => {
  try {
    ...
    const [evi, iocs, mitre, hay, sig, yar, tri, bms, pins, anotes, carnet] = await Promise.all([
...
router.post('/:caseId/generate', authenticate, async (req, res) => {
  try {
    ...
    let narrativeOrigine = origineDuNarratif({ fourni: Boolean(aiNarrative), assiste: req.body?.narrative_ai_assisted });
    if (!aiNarrative && useAI) {
      try {
        const caseCtx = reportAi.buildContext(caseData, {
          ...
          carnet: await lireCarnet(caseId),
        });
        const resolvedModel = await require('../services/aiService').resolveModel(pool).catch(() => undefined);
        aiNarrative = await reportAi.generateNarrative(caseCtx, resolvedModel);
        narrativeOrigine = origineDuNarratif({ genere: Boolean(aiNarrative) });
    ...
    const numerotes = numeroterSources({ narratif: aiNarrative, notes: analystNotes });
    const sources = await resoudreSources(pool, caseId, numerotes.sources);
    ...
    renderReport(doc, {
      ...
      aiNarrative: numerotes.narratif,
      narrativeOrigine,
      analystNotes: numerotes.notes,
      sources,
      activeSections,
