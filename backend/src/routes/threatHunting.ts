...
import { evaluerRegleSigma, requetesBornees, SIGMA_REGLE_TIMEOUT_MS } from '../services/sigmaRegle';
...
router.put('/yara/rules/:id', authenticate, (requireRole as any)('analyst', 'admin'), async (req: express.Request, res: express.Response) => {
  try {
    ...
    if (content !== undefined || is_active === true) {
      const validation = await validateRule(content ?? r.content);
      if (!validation.valid) return res.status(400).json({ error: validation.error });
    }
...
router.post('/sigma/scan-case/:caseId', authenticate, (requireRole as any)('analyst', 'admin'), async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    let matchedSoFar = 0;
    let criticalSoFar = 0;
    const lecture = requetesBornees(pool, SIGMA_REGLE_TIMEOUT_MS);
    for (let i = 0; i < rulesResult.rows.length; i++) {
      ...
      const item = await evaluerRegleSigma({
        pool, lecture, huntMatches, markTimelineForRule, caseId, rule, presentFields,
        avertir: (m: string) => logger.warn(m),
      });
      summary.push(item);
    ...
    const rulesTimedOut = summary.filter(s => s.timed_out).length;
    const rulesUnsupported = summary.filter(s => s.error && !s.unreachable && !s.timed_out).length;
    const userId = (req as AuthRequest).user?.id;
    await auditLog(userId, 'run_sigma_scan_case', 'case', caseId,
      { rules_checked: rulesResult.rows.length, rules_matched: rulesMatched, total_matches: totalMatches,
        rules_unreachable: rulesUnreachable, rules_unsupported: rulesUnsupported, rules_timed_out: rulesTimedOut }, req.ip);
    ...
    send({ type: 'done', case_id: caseId, rules_checked: rulesResult.rows.length, rules_matched: rulesMatched,
           ...
           rules_timed_out: rulesTimedOut, fields_inventoried: presentFields ? presentFields.size : null, summary });
    res.end();
  } catch (e: any) {
