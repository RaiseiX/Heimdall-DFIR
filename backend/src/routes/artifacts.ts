...
import { authenticate, auditLog, requireRole } from '../middleware/auth';
...
router.delete('/:caseId/:ref/notes/:noteId', authenticate, async (req: express.Request, res: express.Response) => {
  try {
    ...
    const database = pool(req);
    const result = await withCaseDeletion(database, caseId, (client: Pick<Pool, 'query'>) => client.query(
      user?.role === 'admin'
    ...
    if (result.rows.length === 0) return res.status(404).json({ error: 'Note introuvable ou non autorisée' });
    await auditLog(user?.id, 'delete_artifact_note', 'case', caseId, { note_id: noteId, artifact_ref: req.params.ref }, req.ip);
    res.json({ deleted: true });
  } catch (e: any) {
    if (e?.code === 'LEGAL_HOLD') return res.status(409).json({ error: 'Suppression interdite : legal hold actif.', code: 'LEGAL_HOLD' });
    if (e?.status === 404) return res.status(404).json({ error: 'Dossier introuvable' });
    res.status(500).json({ error: e.message });
  }
});
