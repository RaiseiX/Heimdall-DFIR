  ...
  try {
    ...
    if (parent.rowCount === 0) {
      const completed = await findCompletedDeletionOperation(pool, {
        operationType: 'delete_parser_result', targetId: resultId, requestedBy: req.user.id,
      });
      if (completed) {
        ...
        return res.json({
          success: true,
          deleted_id: resultId,
          parser_name: context.parser_name || null,
          records_removed: Number(context.record_count || 0),
    ...
    const row = await withCaseDeletion(pool, caseId, async (client: Pick<Pool, 'query'>) => {
      const locked = await client.query(
        `SELECT id, parser_name, record_count FROM parser_results
      ...
      if (locked.rowCount === 0) {
        ...
        if (!completed) throw Object.assign(new Error('Result not found'), { status: 404 });
        const context = typeof completed.context === 'string' ? JSON.parse(completed.context) : (completed.context || {});
        operationId = completed.id;
        return {
          id: resultId,
          parser_name: context.parser_name || null,
          record_count: Number(context.record_count || 0),
          operationId: completed.id,
          replayed: true,
        };
      }
      const operation = await beginDeletionOperation(pool, {
        operationType: 'delete_parser_result',
        caseId,
        targetId: resultId,
        requestedBy: req.user.id,
        requestIp: req.ip,
        items: [{ kind: 'elasticsearch', locator: resultId }],
      });
      ...
      if (operation.status === 'completed') {
        ...
        return {
          id: resultId,
          parser_name: context.parser_name || locked.rows[0].parser_name,
          record_count: Number(context.record_count ?? locked.rows[0].record_count ?? 0),
          operationId: operation.id,
          replayed: true,
        };
      }
      const indexItem = operation.items[0];
      if (indexItem.status !== 'deleted' && indexItem.status !== 'already_absent') {
        await markDeletionItem(pool, operation.id, indexItem.item_key, 'running');
        try {
          await deleteByResultId(caseId, resultId, { strict: true });
          await markDeletionItem(pool, operation.id, indexItem.item_key, 'deleted', 'delete_by_result_id');
        } catch (error) {
          try { await markDeletionItem(pool, operation.id, indexItem.item_key, 'failed', null, safeErrorCode(error)); } catch {}
          throw Object.assign(new Error('Parser result deletion incomplete'), { status: 502, code: 'DELETION_INCOMPLETE' });
        }
      }
      ...
      await markDeletionOperation(client, operation.id, 'completed', null, {
        parser_name: deleted.rows[0].parser_name,
        record_count: deleted.rows[0].record_count,
      });
      return { ...deleted.rows[0], operationId: operation.id, replayed: false };
    });
    if (!row.replayed) {
      await auditLog(req.user.id, 'delete_parser_result', 'case', caseId,
        { result_id: resultId, operation_id: row.operationId }, req.ip);
    }
    res.json({
      success: true,
      deleted_id: row.id,
    ...
    if (operationId && !operationPrepared) {
      try { await markDeletionOperation(getPool(res), operationId, 'incomplete', safeErrorCode(err)); } catch {}
    }
    ...
    if ((err as any)?.code === 'DELETION_INCOMPLETE') {
      (err as any).operationId = operationId;
    }
    next(err);
  }
});
