// Route-level security-guard test for attribution.js — pool + auth mocked, real caseAccessParam
...
const attributionRouter = require('../../../src/routes/attribution');

function makeApp() { const app = express(); app.use(express.json()); app.use('/api/attribution', attributionRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/attribution/c-1');   // attribution GET /:caseId
  expect(res.status).toBe(403);
});
test('elevated user passes the guard (not 403; ELEVATED bypasses the DB check)', async () => {
  mockUser = { id: 'u1', role: 'admin' };
  const res = await request(makeApp(), 'GET', '/api/attribution/c-1');
