// Route-level security-guard test for mitre.js — pool + auth mocked, real caseAccessParam
...
function makeApp() { const app = express(); app.use(express.json()); app.use('/api/mitre', mitreRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/mitre/c-1');
