// Route-level security-guard test for artifacts.ts — pool + auth mocked, real caseAccessParam
...
const artifactsRouter = require('../../../src/routes/artifacts');

function makeApp() { const app = express(); app.use(express.json()); app.use('/api/artifacts', artifactsRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/artifacts/c-1/refs-with-notes');
