// Route-level security-guard test for iocs.js — pool + auth mocked, real caseAccessParam
...
const iocsRouter = require('../../../src/routes/iocs');

function makeApp() { const app = express(); app.use(express.json()); app.use('/api/iocs', iocsRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/iocs/c-1');   // iocs GET /:caseId
  expect(res.status).toBe(403);
});
test('elevated user passes the guard (not 403; ELEVATED bypasses the DB check)', async () => {
  mockUser = { id: 'u1', role: 'admin' };
  const res = await request(makeApp(), 'GET', '/api/iocs/c-1');
