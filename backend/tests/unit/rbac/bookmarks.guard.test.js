// Route-level security-guard test for bookmarks.js — pool, auth, and caseAccess mocked.
let mockUser;
...
function makeApp() { const app = express(); app.use(express.json()); app.use('/api/cases/:caseId/bookmarks', bookmarksRouter); return app; }
...
test('unauthorized user gets 403 and no DB query runs', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/cases/c-1/bookmarks');
