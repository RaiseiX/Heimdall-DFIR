// Route-level security-guard test for soar.ts — pool, auth, and caseAccess mocked.
let mockUser;
...
const soarRouter = require('../../../src/routes/soar');

function makeApp() { const app = express(); app.use(express.json()); app.use('/api/cases/:caseId/soar', soarRouter); return app; }
...
test('unauthorized user gets 403 and no DB query runs', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/cases/c-1/soar/alerts');
