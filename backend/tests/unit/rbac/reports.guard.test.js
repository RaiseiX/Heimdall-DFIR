// Route-level security-guard test for reports.js — pool + auth mocked, real caseAccessParam
...
function makeApp() { const app = express(); app.use(express.json()); app.use('/api/reports', reportsRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'POST', '/api/reports/c-1/ai-draft', {});
