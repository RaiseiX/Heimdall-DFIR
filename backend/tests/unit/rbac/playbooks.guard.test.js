// Route-level security-guard test for playbooks.js — pool + auth mocked, real caseAccessParam
...
const playbooksRouter = require('../../../src/routes/playbooks');

function makeApp() { const app = express(); app.use(express.json()); app.locals.pool = pool; app.use('/api/playbooks', playbooksRouter); return app; }
...
test('non-assigned analyst gets 403 on a :caseId route', async () => {
  ...
  const res = await request(makeApp(), 'GET', '/api/playbooks/cases/c-1');   // playbooks GET /cases/:caseId
  expect(res.status).toBe(403);
});
test('elevated user passes the guard (not 403; ELEVATED bypasses the DB check)', async () => {
  mockUser = { id: 'u1', role: 'admin' };
  const res = await request(makeApp(), 'GET', '/api/playbooks/cases/c-1');
