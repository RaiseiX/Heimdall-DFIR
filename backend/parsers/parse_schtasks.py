...
def parse_dir(base, ecartes=None):
    records = []
        ...
        for name in files:
            ...
            if not is_task_xml(path):
                if ecartes is not None:
                    ecartes.append((path, 'not a task XML'))
                continue
            ...
            if rec:
                records.append(rec)
            elif ecartes is not None:
                ecartes.append((path, 'unreadable XML'))
    return records
...
def main(argv=None):
    ...
    ecartes = []
    records = parse_dir(args.dir, ecartes)
    out_path = os.path.join(args.csv, args.csvf)
    with open(out_path, 'w', newline='', encoding='utf-8') as fh:
        writer = csv.DictWriter(fh, fieldnames=FIELDNAMES, extrasaction='ignore')
        writer.writeheader()
        writer.writerows(records)
    for chemin, raison in ecartes:
        print(f'Skipped ({raison}): {chemin}')
    suffixe = f' ({len(ecartes)} skipped)' if ecartes else ''
    print(f'Parsed {len(records)} scheduled tasks -> {out_path}{suffixe}')
