    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {step === 'idle' && (
        ...
        >
          <div className="text-center">
            ...
            <p className="text-sm mb-3" style={{ color: '#7d8590' }}>ou cliquez pour sélectionner</p>
            {caseObj && (
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginBottom: 10 }}>
              <span style={{ fontSize: 10, fontFamily: 'monospace', padding: '2px 8px', borderRadius: 3, background: '#4d82c018', color: '#4d82c0', border: '1px solid #4d82c030' }}>
      ...
      {step === 'detected' && detected && Object.keys(detected).length > 0 && (
        <div className="fl-card p-4">
          ...
          {isCatScaleCollection ? (
            <div style={{ background: '#0d1117', border: '1px solid #22c55e30', borderRadius: 8, padding: '14px 16px', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#7d8590', marginLeft: 'auto' }}>
                  {detected.catscale?.n || 0} fichiers · {detected.catscale?.sz || '?'}
                </span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 6 }}>
                {Object.entries(CATSCALE_STEPS).map(([key, { label, icon: Icon }]) => (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 10px', borderRadius: 5, background: '#161b22', border: '1px solid #21262d' }}>
      ...
      {step === 'done' && (
        <div className="fl-card p-5" style={{ borderLeft: `3px solid ${catscaleDetail ? '#22c55e' : '#3fb950'}` }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            {catscaleDetail && (
              <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontFamily: 'monospace', color: '#22c55e' }}>
                ...
                {catscaleDetail.os_info && <span style={{ color: '#7d8590' }}>— {catscaleDetail.os_info}</span>}
              </span>
            )}
          </div>
          ...
          {catscaleDetail ? (
            <div className="mb-4">
              <p className="text-xs font-mono mb-3" style={{ color: '#7d8590' }}>
              ...
              {catscaleDetail.artifacts.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 4 }}>
                  {catscaleDetail.artifacts.map((raw, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px', borderRadius: 5, background: '#0d1117', border: `1px solid ${color}22` }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, flexShrink: 0 }} />
                        <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#8b949e', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>
