...
export const evidenceAPI = {
  ...
  deletionPreview: (id) => api.get(`/evidence/${id}/deletion-preview`),
...
export const networkAPI = {
  ...
  authGraph:       (caseId, evidenceId, { services = false } = {}) =>
    api.get(`/network/${caseId}/auth-graph`, { params: { evidence_id: evidenceId, ...(services ? { services: 1 } : {}) } }),
...
export const collectionAPI = {
  ...
  parseLog: (caseId) => api.get(`/collection/${caseId}/parse-log`),
  ...
  processDossier: (caseId, evidenceId, pid, chemin, fenetre) =>
    api.get(`/collection/${caseId}/processes`, { params: {
      evidence_id: evidenceId, with: 'dossier', pid, path: chemin,
      ...(fenetre?.from ? { from: fenetre.from } : {}),
      ...(fenetre?.to ? { to: fenetre.to } : {}),
    } }),
  processEvents: (caseId, evidenceId, pid, name, fenetre) =>
    api.get(`/collection/${caseId}/processes`, { params: {
      evidence_id: evidenceId, with: 'events', pid, name,
      ...(fenetre?.from ? { from: fenetre.from } : {}),
      ...(fenetre?.to ? { to: fenetre.to } : {}),
    } }),
  ...
  files: (caseId, evidenceId, chemin = '') =>
    api.get(`/collection/${caseId}/files`, { params: { evidence_id: evidenceId, path: chemin } }),
  fileContent: (caseId, evidenceId, chemin, params = {}) =>
    api.get(`/collection/${caseId}/file/content`, { params: { evidence_id: evidenceId, path: chemin, ...params } }),
  fileDownload: (caseId, evidenceId, chemin) =>
    api.get(`/collection/${caseId}/file/download`, { params: { evidence_id: evidenceId, path: chemin }, responseType: 'blob' }),
  filesSearch: (caseId, evidenceId, q, chemin = '') =>
    api.get(`/collection/${caseId}/files/search`, { params: { evidence_id: evidenceId, q, path: chemin } }),
  artifactSummary: (caseId, evidenceId) =>
    api.get(`/collection/${caseId}/artifact-summary`, { params: { evidence_id: evidenceId } }),
  fileHive: (caseId, evidenceId, chemin, { key, search } = {}) =>
    api.get(`/collection/${caseId}/file/hive`, { params: { evidence_id: evidenceId, path: chemin, ...(key ? { key } : {}), ...(search ? { search } : {}) } }),
};
