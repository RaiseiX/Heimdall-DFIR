...
export const detectionsAPI = {
  ...
  timestomping:    (id, params, refresh) => api.get(`/cases/${id}/detections/timestomping`,    { params: { ...(params || {}), ...(refresh ? { refresh: 1 } : {}) } }),
  doubleExt:       (id, refresh)         => api.get(`/cases/${id}/detections/double-ext`, { params: refresh ? { refresh: 1 } : {} }),
  beaconing:       (id, params, refresh) => api.get(`/cases/${id}/detections/beaconing`,       { params: { ...(params || {}), ...(refresh ? { refresh: 1 } : {}) } }),
  persistence:     (id, refresh)         => api.get(`/cases/${id}/detections/persistence`, { params: refresh ? { refresh: 1 } : {} }),
  sysmonBehavior:  (id, refresh)         => api.get(`/cases/${id}/detections/sysmon-behavior`, { params: refresh ? { refresh: 1 } : {} }),
  antiForensic:    (id, refresh)         => api.get(`/cases/${id}/detections/anti-forensic`, { params: refresh ? { refresh: 1 } : {} }),
  executionAnomaly:(id, refresh)         => api.get(`/cases/${id}/detections/execution-anomaly`, { params: refresh ? { refresh: 1 } : {} }),
  attackTechniques:(id, refresh)         => api.get(`/cases/${id}/detections/attack-techniques`, { params: refresh ? { refresh: 1 } : {} }),
  vulnDrivers:     (id, refresh)         => api.get(`/cases/${id}/detections/vuln-drivers`, { timeout: 120_000, params: refresh ? { refresh: 1 } : {} }),
