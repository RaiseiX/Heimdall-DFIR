    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
      <div style={{ textAlign: 'center', maxWidth: 340 }}>
        <div style={{
          ...
          <Target size={20} style={{ color: 'var(--fl-muted)' }} strokeWidth={1.5} />
        </div>
        ...
        <div style={{ fontSize: 12, color: 'var(--fl-dim)', lineHeight: 1.5 }}>
          Enrichissez la Kill Chain via les onglets Bookmarks, Techniques MITRE ou Hayabusa
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding: 16, overflow: 'auto', height: '100%', background: 'var(--fl-bg)' }}>
      ...
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ fontSize: 13, color: 'var(--fl-dim)' }}>
          <span style={{ color: 'var(--fl-text)', fontWeight: 700 }}>{data.total_case_techniques}</span> techniques detected in this case
          {data.attributions.length > 0
            ? <> — <span style={{ color: 'var(--fl-text)', fontWeight: 700 }}>{data.attributions.length}</span> correlated APT group(s)</>
            : ' — No APT correlation'}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 20 }}>
        {data.case_techniques.map(t => (
          <span key={t} style={{ fontSize: 10, padding: '2px 6px', background: 'var(--fl-panel)', color: 'var(--fl-dim)', borderRadius: 4, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', border: '1px solid var(--fl-border)' }}>{t}</span>
        ))}
      </div>

      {data.attributions.length === 0 ? (
        <div style={{ textAlign: 'center', color: 'var(--fl-dim)', padding: 32, fontSize: 12 }}>
          No APT correlation found with the current techniques
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {data.attributions.map(apt => (
            <div key={apt.id} style={{ background: 'var(--fl-panel)', border: `1px solid color-mix(in srgb, ${apt.color} 19%, transparent)`, borderRadius: 10, padding: '14px 16px', borderLeft: `3px solid ${apt.color}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--fl-text)' }}>{apt.name}</span>
                    <span style={{ fontSize: 10, padding: '2px 6px', background: apt.color + '20', color: apt.color, borderRadius: 4, border: `1px solid color-mix(in srgb, ${apt.color} 25%, transparent)` }}>{apt.id}</span>
                    <span style={{ fontSize: 10, padding: '2px 6px', background: `color-mix(in srgb, ${confColor[apt.confidence]} 13%, transparent)`, color: confColor[apt.confidence], borderRadius: 4, fontWeight: 700 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {apt.matched_techniques.map(t => (
                  <span key={t} style={{ fontSize: 10, padding: '2px 6px', background: apt.color + '15', color: apt.color, borderRadius: 4, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', border: `1px solid color-mix(in srgb, ${apt.color} 19%, transparent)` }}>{t}</span>
                ))}
              </div>
            </div>
          ))}
