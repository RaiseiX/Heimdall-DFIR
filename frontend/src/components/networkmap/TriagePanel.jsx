...
import { Search, Crosshair, ExternalLink } from 'lucide-react';
...
const ACT_EXPORT = { ...controlStyle(false), alignSelf: 'flex-start' };
...
export default function TriagePanel({ elements, cy, caseId, onPivot }) {
  ...
  const Stat = ({ label, value, color }) => (
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ fontSize: 9, fontFamily: MONO, fontWeight: 700, color: 'var(--fl-muted)' }}>{label}</div>
    </div>
  );
    ...
    <div style={{ width: 264, flexShrink: 0, overflowY: 'auto',
      ...
      <form onSubmit={runSearch} style={{ position: 'relative' }}>
        ...
        <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('networkMap.triage.search_ph')}
          style={{ width: '100%', padding: '7px 9px 7px 28px', borderRadius: 'var(--fl-radius-md)', background: 'var(--fl-bg)', border: '1px solid var(--fl-border)', color: 'var(--fl-text)', fontFamily: MONO, fontSize: 11, outline: 'none' }} />
      </form>
      ...
      <div>
        <div style={{ fontSize: 9, fontFamily: MONO, color: 'var(--fl-muted)', marginBottom: 6 }}>
      ...
      <button type="button" onClick={exportReport} title={t('networkMap.triage.export_title')}
        style={ACT_EXPORT}>
        {t('networkMap.triage.export')}
      </button>
    </div>
  );
}
