...
const ACT = controlStyle(false);
const ACT_NEUTRE = { ...ACT };
const ACT_DANGER = { ...ACT, color: 'var(--fl-danger)' };
const ACT_FAIT   = { ...ACT, color: 'var(--fl-ok)', cursor: 'default' };
const ACT_INERTE = { ...ACT, color: 'var(--fl-subtle)', cursor: 'not-allowed' };
const PIED = {
  padding: '7px 10px', borderTop: '1px solid var(--fl-card)',
...
export default function InvestigationDrawer({ nodeData, caseId, collectionId, allEdges, onClose, onSelectPeer, nodeOverrides, onOverrideType, onResetType, onDeleteManualNode, zoneDeclarations, onDeclareZone, onWithdrawZone, onHideNode }) {
  ...
  return (
    <div style={{ width: 'clamp(380px, 30vw, 540px)', flexShrink: 0, background: 'var(--fl-bg)', borderLeft: '1px solid var(--fl-raised)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '11px 13px 10px', borderBottom: '1px solid var(--fl-card)', flexShrink: 0 }}>
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            >
              {ALL_TYPES.map(t => (
                <option key={t.id} value={t.id} style={{ background: 'var(--fl-bg)', color: t.color }}>
                  {t.label}
                </option>
              ))}
            </select>
        ...
        {declarable && (
          <div style={{ marginTop: 9, paddingTop: 9, borderTop: '1px solid var(--fl-card)' }}>
      ...
      <div style={PIED}>
        {nodeData._manual ? (
          <button type="button" onClick={() => onDeleteManualNode?.(nodeData.id)} style={ACT_DANGER}>
            {t('networkMap.action_delete_node')}
          </button>
        ) : (
          <>
            <button type="button" onClick={goTimeline} disabled={!art.valid}
              style={art.valid ? ACT_NEUTRE : ACT_INERTE}>
              {t('networkMap.action_timeline')}
            </button>
            <button type="button" onClick={flagIoc} disabled={!art.valid || flagged || busy}
              style={flagged ? ACT_FAIT : art.valid ? ACT_DANGER : ACT_INERTE}>
              {flagged ? t('networkMap.flagged') : t('networkMap.action_flag_ioc')}
            </button>
            <button type="button" onClick={pinFinding} disabled={!art.valid || pinned || busy}
              style={pinned ? ACT_FAIT : art.valid ? ACT_NEUTRE : ACT_INERTE}>
