...
export default function ConnectionsTab({ nodeData, allEdges, onSelectPeer }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '6px 10px' }}>
      <div style={{ fontSize: 7, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', marginBottom: 6 }}>
          ...
          <div key={i} style={{ marginBottom: 2 }}>
            <div
              ...
              style={{
                ...
                background: isOpen ? 'var(--fl-card)' : '#0a0f18',
              ...
              onMouseLeave={e => { e.currentTarget.style.background = isOpen ? 'var(--fl-card)' : '#0a0f18'; }}
            >
              ...
              {(edge.label || (edge.protocols && edge.protocols[0])) && (
                <span style={{ fontSize: 7, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '1px 4px',
                  borderRadius: 2, background: '#0c1828', border: '1px solid var(--fl-raised)',
            ...
            {isOpen && (
              <div style={{ background: 'var(--fl-bg)', border: '1px solid var(--fl-card)', borderTop: 'none', borderRadius: '0 0 3px 3px', padding: '8px 10px' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 8 }}>
                  {[
                    ...
                    <div key={label}>
                      <div style={{ fontSize: 7, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', }}>{label}</div>
                ...
                {edge.ports?.length > 0 && (
                  <div style={{ marginBottom: 6 }}>
                    <div style={{ fontSize: 7, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', marginBottom: 3 }}>{t('networkMap.fields.ports')}</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {edge.ports.slice(0, 12).map(p => (
                        <span key={p} style={{ fontSize: 8, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '1px 5px', borderRadius: 2, background: '#0c1828', border: '1px solid var(--fl-raised)', color: 'var(--fl-muted)' }}>{p}</span>
                      ))}
                ...
                {edge.protocols?.length > 0 && (
                  <div style={{ marginBottom: 6 }}>
                    <div style={{ fontSize: 7, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', marginBottom: 3 }}>{t('networkMap.fields.protocols')}</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {edge.protocols.map(p => (
                        <span key={p} style={{ fontSize: 8, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '1px 5px', borderRadius: 2, background: '#0a1820', border: '1px solid var(--fl-raised)', color: 'var(--fl-purple)' }}>{p}</span>
                      ))}
                    </div>
                  </div>
                )}
