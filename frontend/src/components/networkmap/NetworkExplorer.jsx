...
export default function NetworkExplorer({
  ...
  }, [nodeColorOverrides, colorblindMode, mode]);
  ...
  return (
    <div style={{ flex: 1, position: 'relative', background: 'var(--fl-bg)', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: 12, left: 12, display: 'flex', flexDirection: 'column', gap: 2, zIndex: 10 }}>
        {[
          { label: '+', title: t('networkMap.zoom_in'),  fn: zoomIn  },
          { label: '−', title: t('networkMap.zoom_out'), fn: zoomOut },
          { label: '⊡', title: t('networkMap.zoom_fit'), fn: fitAll  },
        ].map(({ label, title, fn }) => (
          <button key={label} type="button" onClick={fn} title={title} style={ACT_ZOOM}>{label}</button>
        ))}
      </div>
    </div>
  );
