...
import { couleursDuTheme, avecAlpha } from './themeColors';

export const PORT_CLASSES = Object.freeze([
  { id: 'cleartext',      teinte: 'danger', cb: '#D55E00', opacity: 0.9 },
  { id: 'notable',        teinte: 'gold',   cb: '#E69F00', opacity: 0.8 },
  { id: 'encrypted',      teinte: 'ok',     cb: '#0072B2', opacity: 0.7 },
  { id: 'infrastructure', teinte: 'accent', cb: '#CC79A7', opacity: 0.7 },
]);

export function portColor(id, colorblindMode = false, couleurs = couleursDuTheme()) {
  const c = PORT_CLASSES.find(p => p.id === id);
  if (!c) return null;
  return colorblindMode ? c.cb : couleurs[c.teinte];
}

export function buildCytoscapeStyle(nodeColorOverrides = {}, colorblindMode = false, couleurs = couleursDuTheme()) {
  const c = couleurs;
  const styles = [
    {
      selector: 'node',
    ...
    },
    {
      ...
      style: {
        ...
        'width': 1,
        'height': 'data(h)',
        'background-color': c.surfaceActive,
    ...
    },
    {
      ...
      style: {
        'shape': 'roundrectangle',
        'background-color': avecAlpha(c.purple, '08'),
        'border-width': 1,
        'border-style': 'dashed',
        'border-color': c.surfaceActive,
        'label': 'data(label)',
        'font-size': 18,
        'color': c.text,
        'font-weight': 700,
    ...
    },
    {
      ...
      style: {
        'width': 'data(_w)',
        'line-color': avecAlpha(c.purple, '40'),
        'target-arrow-color': avecAlpha(c.purple, '40'),
