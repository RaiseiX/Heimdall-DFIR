...
export default function TagsTab({ record: r }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div>
        <div style={{ fontSize: 8, fontWeight: 700, color: 'var(--fl-muted)', marginBottom: 7 }}>Confidence</div>
      ...
      <div>
        <div style={{ fontSize: 8, fontWeight: 700, color: 'var(--fl-muted)', marginBottom: 7 }}>Categories</div>
      ...
      {entry.tags.length > 0 && (
        <div>
          <div style={{ fontSize: 8, fontWeight: 700, color: 'var(--fl-subtle)', marginBottom: 5 }}>Filter by tag</div>
          {entry.tags.map(key => {
