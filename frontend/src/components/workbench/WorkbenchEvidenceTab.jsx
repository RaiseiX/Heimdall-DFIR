...
export default function WorkbenchEvidenceTab({ caseId }) {
  ...
  const exportJSON = () => {
    ...
    a.href = url; a.download = `pinned-evidence-${caseId}-${Date.now()}.json`;
...
<h1>HEIMDALL DFIR — Pinned evidence report</h1>
<div class="cover">
...
<footer>
  Generated by Heimdall DFIR — Pinned evidence · Chain of custody: each evidence entry lists pin_id, pinned_at UTC, pinned_by, collection_timeline_id, dedupe_hash.
</footer>
</body></html>`;
    ...
    <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>
      <div style={{
        ...
        <button onClick={() => { if (confirm(`Remove ${pins.length} pins from this case?`)) clear(caseId); }}
          title="Clear pinned evidence" style={{ ...iconBtn, color: 'var(--fl-danger)' }}>
          <Trash2 size={12} />
        </button>
      </div>
          ...
          return (
            <div key={p.pin_id} style={{
              ...
              <div style={{
                ...
                <button onClick={() => unpin(caseId, p.pin_id)} title="Retirer des preuves épinglées"
