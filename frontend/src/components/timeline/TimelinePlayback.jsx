...
const SPEEDS = [
  { label: '0.5x', evtPerSec: 0.5 },
  { label: '1x',   evtPerSec: 1   },
  { label: '2x',   evtPerSec: 2   },
  { label: '5x',   evtPerSec: 5   },
  { label: '10x',  evtPerSec: 10  },
];
  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          padding: '3px 10px', borderRadius: 4, fontSize: 10, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)',
          background: playing ? 'rgba(239,68,68,0.15)' : 'rgba(77,130,192,0.15)',
          color: playing ? 'var(--fl-danger)' : 'var(--fl-accent)',
      ...
      <div style={{ display: 'flex', gap: 2 }}>
        {SPEEDS.map((s, i) => (
          <button key={i} onClick={() => setSpeedIdx(i)}
            style={{
              padding: '2px 6px', borderRadius: 3, fontSize: 9, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)',
              background: speedIdx === i ? 'rgba(77,130,192,0.2)' : 'transparent',
              color: speedIdx === i ? 'var(--fl-accent)' : '#2a5a8a',
      ...
      >
        ...
        <div style={{ position: 'absolute', top: 5, left: 0, width: `${pct * 100}%`, height: 4, background: 'var(--fl-accent)', borderRadius: 2 }} />
        <div style={{
          position: 'absolute', top: 1, left: `calc(${pct * 100}% - 6px)`,
          width: 12, height: 12, borderRadius: '50%', background: 'var(--fl-accent)',
          border: '2px solid #060b14',
        }} />
      </div>

      <span style={{ fontSize: 9, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: '#2a5a8a', flexShrink: 0 }}>
        {cursor + 1} / {total}
      </span>

      {current && (
        <span style={{ fontSize: 9, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: '#7abfff', flexShrink: 0, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
