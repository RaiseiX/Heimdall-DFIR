...
import { Copy, CheckCheck, ChevronDown, ChevronRight, AlertTriangle, Search } from 'lucide-react';
import { useTheme } from '../../utils/theme';

function askAi(topic) {
  const prompt = `Explique le sujet de forensique réseau « ${topic.title} » : ce qu'il couvre, comment l'analyser, et ce qu'il faut chercher dans une investigation DFIR.`;
...
function TopicCard({ topic, search, compact }) {
  const T = useTheme();
      ...
      <button onClick={() => setOpen(o => !o)} className="w-full text-left"
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontWeight: 700, fontSize: compact ? 12 : 13, color: 'var(--fl-text)' }}>{topic.title}</span>
          ...
          <button onClick={(e) => { e.stopPropagation(); askAi(topic); }} title="Demander à l'IA"
            style={{
        ...
        <p style={{ fontSize: 11, marginTop: 5, marginLeft: 22, color: 'var(--fl-muted)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', lineHeight: 1.5 }}>{topic.summary}</p>
      </button>
        ...
        <div style={{ background: 'var(--fl-bg)', padding: '14px 16px', borderTop: '1px solid var(--fl-border)' }}>
          {topic.content.map((section, si) => (
            <div key={si} style={{ marginBottom: 14 }}>
              <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--fl-accent)', marginBottom: 7 }}>{section.label}</div>
          ...
          {topic.iocs && topic.iocs.length > 0 && (
            <div>
              <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--fl-accent)', marginBottom: 7 }}>IOCs réseau</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
...
export const DOC_INDEX = TOPICS.map(t => ({ title: t.title, sub: t.category }));
...
export default function NetworkForensicsDoc({ search }) {
  ...
  return (
    <div style={{ padding: '26px 34px', maxWidth: 960 }}>
      <div style={{ marginBottom: 18 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--fl-dim)', marginTop: 5, fontFamily: 'var(--f-ui, "Inter", sans-serif)' }}>
          {search || catFilter !== 'Tous'
            ...
            : `${TOPICS.length} sections · PCAP · C2 · DNS · Lateral Movement · Exfiltration · Outils`}
        </p>
      </div>
      ...
      {filtered.map(t => <TopicCard key={t.id} topic={t} search={search} compact={compact} />)}

      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '64px 0', color: 'var(--fl-muted)' }}>
          <Search size={28} style={{ opacity: 0.35, marginBottom: 10 }} />
          <p style={{ fontSize: 13, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>Aucun contenu ne correspond à « {search} »</p>
        </div>
      )}
    </div>
  );
