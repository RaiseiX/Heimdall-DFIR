...
const LOGON_TYPES = [
  ...
  { type: '3',  name: 'Network',           desc: 'Accès réseau (SMB, WMI)',                          signal: 'Surveiller sources inhabituelles', level: 'warn' },
  ...
  { type: '8',  name: 'NetworkCleartext',  desc: 'Credentials en clair sur le réseau',               signal: 'Alerte immédiate', level: 'danger' },
  { type: '9',  name: 'NewCredentials',    desc: 'RunAs /netonly',                                   signal: 'Surveiller', level: 'warn' },
  { type: '10', name: 'RemoteInteractive', desc: 'RDP / Terminal Services',                          signal: 'Surveiller sources externes', level: 'warn' },
    ...
    <table style={tableStyle}>
      <thead>
        <tr style={{ background: 'var(--fl-card)' }}>
          <th style={headStyle(false)}>Event ID</th>
          {showCanal && (
            <th style={headStyle(false)}>Canal</th>
          )}
          <th style={headStyle(false)}>Description</th>
          <th style={headStyle(false)}>MITRE</th>
          <th style={headStyle(false)}>Criticité</th>
        </tr>
      </thead>
      <tbody>
        {filtered.map((e, i) => (
      ...
      <Section title="Fichiers EVTX — Priorité de collecte" subtitle={`${EVTX_FILES.length} fichiers`}>
        <div style={{ padding: '16px 18px' }}>
          <table style={tableStyle}>
            <thead>
              <tr>
                <th style={{ ...headStyle(false), textAlign: 'left', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: 10,
            ...
            <tbody>
              {LOGON_TYPES.filter(lt => !search ||
                ...
                <tr key={lt.type} style={{ background: i % 2 === 0 ? 'transparent' : `color-mix(in srgb, ${T.panel} 53%, transparent)`,
                  ...
                  <td style={{ ...cellStyle(), fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: 11, color: T.text }}>{lt.name}</td>
                  <td style={{ ...cellStyle(), color: T.text }}>{lt.desc}</td>
                  <td style={{ ...cellStyle(), color: lt.level === 'danger' ? 'var(--fl-danger)' : lt.level === 'warn' ? 'var(--fl-warn)' : T.muted, fontSize: 11 }}>{lt.signal}</td>
                </tr>
              ))}
            </tbody>
          </table>
