                ...
                <div style={{ display: 'grid', gridTemplateColumns: m.length === 4 ? '1fr 1fr 1fr 1fr' : `repeat(${m.length}, auto)`, gap: '10px 18px', justifyContent: 'start' }}>
                  {m.map(([label, value, c]) => (
                    <div key={label} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      ...
                      <span style={{ fontSize: 9, fontFamily: MONO, color: 'var(--fl-subtle)' }}>{label}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            return (
              <div key={row.id} style={{ borderRadius: 10, border: '1px solid var(--fl-border)', background: 'var(--fl-panel)', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px' }}>
                  ...
                  <span style={{ fontFamily: MONO, fontSize: 10, color: 'var(--fl-muted)', minWidth: 72 }}>{TYPE_LABELS[row.type] || row.type}</span>
...
function DockerTab() {
  ...
  const Panel = ({ title, chartData, dataKey, warnAt }) => (
    <div style={{ background: 'var(--fl-card)', border: '1px solid var(--fl-border)', borderRadius: 10, padding: '14px 16px' }}>
      <p style={{ fontSize: 10.5, fontWeight: 600, fontFamily: MONO, color: 'var(--fl-muted)', marginBottom: 12 }}>{title}</p>
                ...
                <thead>
                  <tr style={{ background: 'var(--fl-panel)', borderBottom: '1px solid var(--fl-border)' }}>
                    {[t('admin.col_timestamp'), t('admin.col_user'), t('admin.logs.method'), 'Path', t('admin.col_status'), 'ms', 'IP'].map(h => (
                      <th key={h} style={{ ...headStyle(false), textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'var(--fl-muted)', }}>{h}</th>
                    ))}
                  </tr>
                </thead>
              </table>
                      ...
                      {accessVirt.getVirtualItems().map(vi => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ ...cellStyle(), overflow: 'hidden' }}>
                              <span style={{ fontSize: 9, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontWeight: 700, padding: '1px 6px', borderRadius: 3, background: `color-mix(in srgb, ${mc} 8%, transparent)`, color: mc, border: `1px solid color-mix(in srgb, ${mc} 16%, transparent)`, }}>
                                {row.method}
                              </span>
                            </td>
                  ...
                  {logVirt.getVirtualItems().map(vi => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span style={{ color: lc, fontWeight: 700, fontSize: 9 }}>
                          {entry.level || '?'}
                        </span>
