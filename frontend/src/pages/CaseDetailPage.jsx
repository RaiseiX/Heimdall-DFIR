                    ...
                    {(ev.evidence_type === 'memory' || /\.(raw|mem|vmem|lime|dmp)$/i.test(ev.original_filename || ev.name || '')) && (
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: ev.volweb_status === 'ready'     ? 'rgba(34,197,94,0.10)'
                                    : ev.volweb_status === 'processing' ? 'rgba(200,157,29,0.10)'
                                    : ev.volweb_status === 'error'      ? 'rgba(218,54,51,0.10)'
                                    : 'rgba(139,114,214,0.10)',
                          color: ev.volweb_status === 'ready'     ? '#22c55e'
                               : ev.volweb_status === 'processing' ? '#c89d1d'
                               : ev.volweb_status === 'error'      ? '#da3633'
                               : '#8b72d6',
                          border: `1px solid ${
                            ev.volweb_status === 'ready'     ? 'rgba(34,197,94,0.30)'
                          : ev.volweb_status === 'processing' ? 'rgba(200,157,29,0.30)'
                          : ev.volweb_status === 'error'      ? 'rgba(218,54,51,0.30)'
                          : 'rgba(139,114,214,0.30)'}`,
                          flexShrink: 0, fontWeight: 700,
                        }}
                        title={
                          ev.volweb_status === 'ready'      ? 'Ouvrir dans VolWeb (SSO)' :
                          ev.volweb_status === 'processing' ? 'Analyse Volatility 3 en cours — cliquer pour ouvrir VolWeb' :
                          ev.volweb_status === 'error'      ? 'Erreur VolWeb — cliquer pour réessayer' :
                          'Ouvrir dans VolWeb — Analyse mémoire Volatility 3'
                        }
                      >
                        <Cpu size={11} />
                        {ev.volweb_status === 'processing'
                          ? `Volatility… ${volwebProgress[ev.id] ? `${volwebProgress[ev.id].volweb_raw_status ?? volwebProgress[ev.id].pct}%` : ''}`
                          : ev.volweb_status === 'uploading' ? 'Envoi VolWeb…'
                          : ev.volweb_status === 'ready'     ? 'VolWeb ✓'
                          : ev.volweb_status === 'error'     ? 'VolWeb ✗'
                          : 'VolWeb ↗'}
                      </button>
                          ...
                          <div style={{
                            ...
                            <span style={{ fontSize: 9, fontFamily: 'monospace', color: '#7d8590' }}>
                              {isUploading
                                ? 'Envoi vers VolWeb…'
                                : p.tasks_total > 0
                                  ? `${p.tasks_done}/${p.tasks_total} plugins · ${pct}%`
                                  : `Volatility 3 · ${pct}%`}
                            </span>
                          </div>
                        );
