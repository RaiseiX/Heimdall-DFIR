...
import { Activity, AlertTriangle, BookOpen, CalendarDays, Check, ChevronRight, Clock, Copy, Cpu, Crosshair, Download, FileDown, FileJson, FolderOpen, Globe, Info, Loader2, Lock, MessageCircle, Paperclip, Pencil, Plus, RefreshCw, Shield, Star, Trash2, X } from 'lucide-react';
...
import ParseTerminal from '../components/collection/ParseTerminal';
import SuppressionPreuve from '../components/evidence/SuppressionPreuve';
...
import ReportWritingPane from '../components/reports/ReportWritingPane';
import { preRemplir, narratifDuDoc, iaUtilisee, noteAnalyste } from '../components/reports/reportWriting';
...
import { faitsDePiece, actionsDePiece, octetsArchiveDe } from './evidenceRow';
...
const TON_DU_FAIT = { ecart: 'fl-piece-ecart', alerte: 'fl-piece-alerte' };
const CACHE = { display: 'none' };
const ACTION_SORTIE = { color: 'var(--fl-text)', fontWeight: 600 };
const SORTIE_SURVOL = {
  onMouseEnter: (e) => { e.currentTarget.style.color = 'var(--fl-accent)'; },
  onMouseLeave: (e) => { e.currentTarget.style.color = 'var(--fl-text)'; },
};

function texteDuFait(seg, t, langue, analyse) {
  const n = (v) => Number(v).toLocaleString(langue);
  switch (seg.cle) {
    case 'type': return seg.valeur;
    case 'taille': return fmtSize(seg.valeur);
    case 'archive': return t('casedetail.fact_archive', { taille: fmtSize(seg.valeur) });
    case 'extrait': return t('casedetail.fact_extrait', { taille: fmtSize(seg.valeur) });
    case 'verse': return t('casedetail.fact_verse', { date: new Date(seg.valeur).toLocaleDateString(langue) });
    case 'quarantaine': return t('casedetail.fact_quarantaine');
    case 'parse': return t('casedetail.fact_parse', { n: n(seg.valeur) });
    case 'indexe': return t('casedetail.fact_indexe', { n: n(seg.valeur) });
    case 'ecart': return seg.valeur > 0
      ? t('casedetail.fact_ecart_manquant', { n: n(seg.valeur) })
      : t('casedetail.fact_ecart_surplus', { n: n(-seg.valeur) });
    case 'non-parse': return analyse ? t('casedetail.analyzing') : t('casedetail.fact_non_parse');
    default: return '';
  }
}
  ...
  return (
    <div style={{ borderRadius: 3, border: '1px solid color-mix(in srgb, var(--fl-danger) 19%, transparent)', background: '#1a0f0f', overflow: 'hidden' }}>
      <div style={{ display: 'flex', borderBottom: '1px solid color-mix(in srgb, var(--fl-danger) 15%, transparent)' }}>
        ...
        <span style={{ marginLeft: 'auto', padding: '5px 10px', fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'color-mix(in srgb, var(--fl-danger) 38%, transparent)', alignSelf: 'center' }}>SUSPECT</span>
      </div>
      <div style={{ padding: '8px 10px', maxHeight: 200, overflowY: 'auto' }}>
        {loadingPreview ? (
          <div style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-muted)', textAlign: 'center', padding: '12px 0' }}>{t('casedetail.hex_loading')}</div>
        ) : previewError ? (
          <div style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-danger)' }}>{previewError}</div>
        ) : activeTab === 'hex' ? (
          <pre style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'color-mix(in srgb, var(--fl-danger) 56%, transparent)', margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-all', lineHeight: 1.6 }}>{hexData || t('casedetail.hex_empty')}</pre>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {(stringsData || []).filter(Boolean).map((s, i) => (<span key={i} style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-gold)', padding: '1px 0' }}>{s}</span>))}
            {(!stringsData || stringsData.filter(Boolean).length === 0) && (<span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-muted)' }}>{t('casedetail.no_strings')}</span>)}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE,
...
function TabCount({ children, color = 'var(--fl-muted)' }) {
  return (
    <span style={{ marginLeft: 5, fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color }}>
      {children}
    </span>
  );
}
...
export default function CaseDetailPage({ user }) {
  ...
  const [conflitsIa, setConflitsIa] = useState([]);
  const [narratifIa, setNarratifIa] = useState(null);
  ...
  const generateAiDraft = async () => {
    const doc = reportDocRef.current;
    if (!doc) return;
    setAiLoading(true); setAiError(''); setConflitsIa([]); setNarratifIa(null);
    try {
      const { reportsAPI: rAPI } = await import('../utils/api');
      const note = noteAnalyste(doc);
      const { data } = await rAPI.aiDraft(c.id, note ? { notes: note } : {});
      const narratif = data.narrative || {};
      const { conservees } = preRemplir(doc, narratif);
      if (conservees.length) { setNarratifIa(narratif); setConflitsIa(conservees); }
    } catch (e) {
  ...
  const remplacerConflitsIa = () => {
    preRemplir(reportDocRef.current, narratifIa, { forcer: conflitsIa });
    setConflitsIa([]); setNarratifIa(null);
  };
  const garderConflitsIa = () => { setConflitsIa([]); setNarratifIa(null); };
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const doc = reportDocRef.current;
      const note = noteAnalyste(doc);
      if (note) opts.notes = note;
      opts.use_ai = aiEnabled;
      const narratif = narratifDuDoc(doc);
      if (narratif) {
        opts.ai_narrative = narratif;
        opts.narrative_ai_assisted = iaUtilisee(doc);
      }
      ...
      setReportId(null);
      setReportError(e?.response?.data?.error || t('casedetail.report_generation_failed'));
    }
    setGenerating(false);
  };
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      setReportError(e?.response?.data?.error || t('casedetail.report_download_failed'));
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ position: 'sticky', top: 36, zIndex: 100, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          <button onClick={() => setStatusModal('_pick')} title={t('casedetail.change_status')} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 9px', borderRadius: 3, cursor: 'pointer', background: `color-mix(in srgb, ${SM[c.status]?.c || 'var(--fl-dim)'} 11%, transparent)`, color: SM[c.status]?.c || 'var(--fl-dim)', border: `1px solid color-mix(in srgb, ${SM[c.status]?.c || 'var(--fl-dim)'} 26%, transparent)`, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontWeight: 600, flexShrink: 0 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'currentColor', flexShrink: 0 }} />
            {SM[c.status]?.l || c.status}<span style={{ fontSize: FS_MINI, opacity: 0.6 }}>▾</span>
          </button>

          <span style={{ width: 1, height: 14, background: 'var(--fl-sep)', flexShrink: 0 }} />

          <Clock size={10} style={{ color: 'var(--fl-subtle)', flexShrink: 0 }} />
          <span style={{ fontSize: FS_MICRO, color: 'var(--fl-subtle)', whiteSpace: 'nowrap', flexShrink: 0 }}>{t('casedetail.opened_on', { date: new Date(c.created_at).toLocaleDateString(i18n.language) })}</span>
          {c.investigator_name && (
            <><span style={{ color: 'var(--fl-sep)', fontSize: FS_TEXTE }}>·</span>
            <span style={{ fontSize: FS_MICRO, color: 'var(--fl-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>{c.investigator_name}</span></>
          )}
          <span style={{ color: 'var(--fl-sep)', fontSize: FS_TEXTE }}>·</span>
          {editDeadline ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              <input type="datetime-local" value={deadlineVal} onChange={e => setDeadlineVal(e.target.value)} className="fl-input" style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '0 4px', height: 20 }} autoFocus />
            ...
            <button onClick={() => { setDeadlineVal(c.report_deadline ? c.report_deadline.slice(0,16) : ''); setEditDeadline(true); }} title={t('casedetail.edit_deadline')} style={{ display: 'flex', alignItems: 'center', gap: 3, background: 'none', border: 'none', cursor: 'pointer', padding: '1px 4px', borderRadius: 3, flexShrink: 0, color: c.report_deadline && new Date(c.report_deadline) < new Date(Date.now() + 48*3600*1000) ? 'var(--fl-danger)' : 'var(--fl-muted)' }}>
              <CalendarDays size={9} />
              <span style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>{c.report_deadline ? new Date(c.report_deadline).toLocaleDateString(i18n.language) : t('casedetail.deadline')}</span>
            ...
            <div style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {showTimeTooltip && caseTimeStats.analysts?.length > 0 && (
                <div style={{ position: 'absolute', left: 0, top: '100%', marginTop: 6, zIndex: 500, background: 'var(--fl-panel)', border: '1px solid var(--fl-border)', borderRadius: 3, padding: '8px 12px', minWidth: 200, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>
                  <div style={{ fontSize: FS_MINI, color: 'var(--fl-subtle)', marginBottom: 6 }}>{t('casedetail.analytic_time')}</div>
                  {caseTimeStats.analysts.map(a => (
                    <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: FS_TEXTE, padding: '2px 0', color: 'var(--fl-dim)' }}>
                      <span>{a.full_name || a.username}</span>
                      <span style={{ color: 'var(--fl-text)', fontWeight: 600 }}>{fmtDuration(a.total_seconds)}</span>
                    </div>
                  ))}
                  <div style={{ borderTop: '1px solid var(--fl-sep)', marginTop: 5, paddingTop: 5, display: 'flex', justifyContent: 'space-between', fontSize: FS_TEXTE }}>
          ...
          {presenceUsers.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }} title={presenceUsers.map(u => u.full_name || u.username).join(', ')}>
              {presenceUsers.slice(0, 4).map((u, i) => {
                ...
                return (
                  <div key={u.id + i} title={u.full_name || u.username} style={{ width: 18, height: 18, borderRadius: '50%', background: `color-mix(in srgb, ${col} 13%, transparent)`, border: `1px solid color-mix(in srgb, ${col} 38%, transparent)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontWeight: 700, color: col, marginLeft: i > 0 ? -5 : 0, zIndex: 4 - i }}>
                    {ini}
                  </div>
                );
              })}
              {presenceUsers.length > 4 && <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--fl-card)', border: '1px solid var(--fl-sep)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-subtle)', marginLeft: -5 }}>+{presenceUsers.length - 4}</div>}
        ...
        <div style={{ display: 'flex', alignItems: 'stretch', flex: 1, overflow: 'auto', scrollbarWidth: 'none' }}>
          ...
          {selEv && !shellCtx.insideCollectionLayout && (
            <>
              <span style={{ color: 'var(--fl-border)', fontSize: FS_TEXTE, alignSelf: 'center', margin: '0 1px', flexShrink: 0 }}>›</span>
              <div style={{
                display: 'flex', alignItems: 'center', gap: 5, padding: '0 8px',
                fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_MICRO, color: 'var(--fl-dim)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0, paddingLeft: 8, borderLeft: '1px solid var(--fl-sep)', marginLeft: 4 }}>
          ...
          {user?.role === 'admin' && c.legal_hold && (
            <span title={t('casedetail.legal_hold_active_title')} style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontWeight: 700, padding: '1px 6px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-danger) 10%, transparent)', color: 'var(--fl-danger)', border: '1px solid color-mix(in srgb, var(--fl-danger) 30%, transparent)' }}>
              <Lock size={8} /> HOLD
            </span>
          )}
          <div style={{ position: 'relative' }}>
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: 3, background: showActionsMenu ? 'var(--fl-card)' : 'transparent', border: `1px solid ${showActionsMenu ? 'var(--fl-border)' : 'var(--fl-sep)'}`, color: 'var(--fl-muted)', cursor: 'pointer', fontSize: FS_TITRE, letterSpacing: 1 }}
            >···</button>
            {showActionsMenu && (
              <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, zIndex: 600, background: 'var(--fl-panel)', border: '1px solid var(--fl-border)', borderRadius: 3, padding: 4, minWidth: 180 }}>
                {user?.role === 'admin' && (c.legal_hold ? (
                  <>
                    <button onClick={() => { setShowActionsMenu(false); downloadManifest(); }} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 10px', background: 'none', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-purple)' }}><FileJson size={11} />{t('casedetail.manifest')}</button>
                    <button onClick={() => { setShowActionsMenu(false); setLegalHoldModal('disable'); }} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 10px', background: 'none', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-warn)' }}><Lock size={11} />{t('casedetail.lift_hold')}</button>
                  </>
                ) : (
                  <button onClick={() => { setShowActionsMenu(false); setLegalHoldModal('enable'); }} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 10px', background: 'none', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}><Lock size={11} />{t('casedetail.legal_hold')}</button>
                ))}
                <button onClick={() => { setShowActionsMenu(false); exportRGPD(); }} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 10px', background: 'none', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}><Lock size={11} />{t('casedetail.export_rgpd')}</button>
                {user?.role === 'admin' && (
                  <>
                    <div style={{ height: 1, background: 'var(--fl-sep)', margin: '3px 6px' }} />
                    <button onClick={() => { setShowActionsMenu(false); setShowHardDelete(true); setHardDeleteConfirm(''); }} style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', padding: '5px 10px', background: 'none', border: 'none', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-danger)' }}><Trash2 size={11} />{t('casedetail.destroy')}</button>
                  </>
                )}
              </div>
            )}
      ...
      {tab === 'notebook' && <NotebookPanel caseId={id} socket={socket} />}

      {tab === 'audit' && (
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="flex items-center mb-4" style={{ gap: 8 }}>
            <Icon name="ScrollText" size={14} style={{ color: 'var(--fl-accent)' }} />
            <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-dim)' }}>{t('casedetail.audit_log')}</span>
            {auditRows.length > 0 && <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', padding: '1px 6px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-accent) 9%, transparent)', color: 'var(--fl-accent)', border: '1px solid color-mix(in srgb, var(--fl-accent) 19%, transparent)' }}>{auditRows.length}</span>}
          </div>
          {auditLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 16px', gap: 8 }}>
              ...
              <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)' }}>{t('casedetail.audit_empty')}</span>
            </div>
          ) : (
                ...
                <thead>
                  <tr style={{ background: 'var(--fl-bg)', borderBottom: '1px solid var(--fl-border)' }}>
                    {[[t('casedetail.col_date'), 150], [t('casedetail.col_actor'), 150], [t('casedetail.col_action'), null], [t('casedetail.col_entity'), 120], ['IP', 120]].map(([l, w]) => (
                      <th key={l} style={{ textAlign: 'left', padding: '7px 10px', width: w || undefined, fontSize: FS_MINI, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)', fontWeight: 600, whiteSpace: 'nowrap' }}>{l}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {auditRows.map((r, i) => {
                    ...
                    return (
                      <tr key={r.id || i}>
                        <td style={{ ...td, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-dim)', whiteSpace: 'nowrap' }}>{r.created_at ? new Date(r.created_at).toLocaleString(i18n.language) : '—'}</td>
                        <td style={{ ...td, fontSize: FS_TEXTE, color: 'var(--fl-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 150 }}>{r.user_name || r.username || t('dashboard.system_actor')}</td>
                        <td style={{ ...td, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-accent)' }}>{(r.action || '').replace(/_/g, ' ')}</td>
                        <td style={{ ...td, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)' }}>{(r.entity_type || '').replace(/_/g, ' ') || '—'}</td>
                        <td style={{ ...td, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-subtle)', whiteSpace: 'nowrap' }}>{r.ip_address || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-dim)', fontWeight: 700 }}>IOCs</span>
                <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', padding: '1px 7px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-accent) 9%, transparent)', color: 'var(--fl-accent)', border: '1px solid color-mix(in srgb, var(--fl-accent) 19%, transparent)' }}>{caseIOCs.length}</span>
              </div>
              <button onClick={() => setShowAddIoc(v => !v)}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 3, cursor: 'pointer', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 600,
            ...
            {showAddIoc && (
              <div style={{ marginBottom: 16, padding: 14, border: '1px solid var(--fl-border)', borderRadius: 3, background: 'var(--fl-panel)', display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                <select value={newIoc.ioc_type} onChange={e => setNewIoc(s => ({ ...s, ioc_type: e.target.value }))}
                  style={{ padding: '7px 9px', borderRadius: 3, background: 'var(--fl-input-bg)', border: '1px solid var(--fl-border)', color: 'var(--fl-text)', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, cursor: 'pointer' }}>
                ...
                <input autoFocus value={newIoc.value} onChange={e => setNewIoc(s => ({ ...s, value: e.target.value }))} placeholder={t('casedetail.ioc_value_ph')}
                  ...
                  style={{ flex: '1 1 240px', minWidth: 200, padding: '7px 10px', borderRadius: 3, background: 'var(--fl-input-bg)', border: '1px solid var(--fl-border)', color: 'var(--fl-text)', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, outline: 'none' }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)' }}>{t('casedetail.severity_abbr')}</span>
                  <input type="number" min="1" max="10" value={newIoc.severity} onChange={e => setNewIoc(s => ({ ...s, severity: e.target.value }))}
                    style={{ width: 54, padding: '7px 8px', borderRadius: 3, textAlign: 'right', background: 'var(--fl-input-bg)', border: '1px solid var(--fl-border)', color: 'var(--fl-text)', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE }} />
                </div>
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-dim)', cursor: 'pointer' }}>
                ...
                <input value={newIoc.description} onChange={e => setNewIoc(s => ({ ...s, description: e.target.value }))} placeholder={t('casedetail.description_optional_ph')}
                  style={{ flex: '1 1 180px', minWidth: 140, padding: '7px 10px', borderRadius: 3, background: 'var(--fl-input-bg)', border: '1px solid var(--fl-border)', color: 'var(--fl-text)', fontFamily: 'var(--f-ui, sans-serif)', fontSize: FS_TEXTE, outline: 'none' }} />
                <button onClick={addIoc} disabled={!newIoc.value.trim() || addingIoc}
                  style={{ padding: '7px 14px', borderRadius: 3, cursor: !newIoc.value.trim() || addingIoc ? 'not-allowed' : 'pointer', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 600,
                ...
                return (
                  <button key={tb.key} onClick={() => setIocVerdictFilter(tb.key)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 11px', background: 'none', border: 'none', cursor: 'pointer',
                      fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 600,
                    ...
                    {tb.label}
                    <span style={{ fontSize: FS_MICRO, color: active ? 'var(--fl-dim)' : 'var(--fl-subtle)', fontFeatureSettings: '"tnum"' }}>{tb.count}</span>
                  </button>
                );
              })}
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)' }}>
            ...
            {caseIOCs.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px 16px', gap: 12 }}>
                ...
                <span style={{ fontFamily: 'var(--f-display, var(--f-sans))', fontSize: FS_TITRE, fontWeight: 700, color: 'var(--fl-text)', letterSpacing: '-0.01em' }}>{t('casedetail.no_iocs')}</span>
              </div>
            ) : (
                        ...
                        <tr key={ioc.id}>
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, width: 36 }}>
                              <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 28, borderRadius: 3, fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 700, fontFeatureSettings: '"tnum"', background: `color-mix(in srgb, ${sevColor} 12%, transparent)`, color: sevColor, border: `1px solid color-mix(in srgb, ${sevColor} 28%, transparent)` }}>
                                {sev}
                              </span>
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', padding: '1px 7px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-purple) 10%, transparent)', color: 'var(--fl-purple)', border: '1px solid color-mix(in srgb, var(--fl-purple) 22%, transparent)', }}>
                                {TYPE_LABEL[ioc.ioc_type] || ioc.ioc_type}
                              </span>
                              {ioc.is_malicious && (
                                <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', padding: '1px 7px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-danger) 10%, transparent)', color: 'var(--fl-danger)', border: '1px solid color-mix(in srgb, var(--fl-danger) 22%, transparent)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            ...
                            <div onClick={() => navigator.clipboard?.writeText(ioc.value)} title={t('casedetail.copy_value_title', { value: ioc.value })}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 600, cursor: 'pointer', wordBreak: 'break-all',
                          ...
                          <td style={{ color: 'var(--fl-dim)', fontSize: FS_TEXTE, maxWidth: 240, lineHeight: 1.45 }} title={ioc.description || ''}>
                            {ioc.description
                          ...
                          <td>
                            {ioc.enriched_at ? (
                              <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-ok)', display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 8px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-ok) 9%, transparent)', border: '1px solid color-mix(in srgb, var(--fl-ok) 20%, transparent)' }}>
                              ...
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-subtle)' }}>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                              {(ioc.tags || []).length === 0
                                ? <span style={{ color: 'var(--fl-subtle)', fontSize: FS_TEXTE }}>—</span>
                                : (ioc.tags || []).map(tag => (
                                  <span key={tag} style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, monospace)', padding: '2px 7px', borderRadius: 3, background: 'var(--fl-raised)', border: '1px solid var(--fl-border)', color: 'var(--fl-dim)' }}>{tag}</span>
                                ))}
                            </div>
                          </td>
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
                              <button onClick={() => handleIocEnrich(ioc)} disabled={isEnriching} title={t('iocs.enrich_title')}
                                style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 3, cursor: isEnriching ? 'wait' : 'pointer', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_MICRO, fontWeight: 600,
      ...
      {tab === 'evidence' && (
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          {parseProg?.active && !showImportPanel && (
            <ParsingMonitor
              caseId={id}
              parsers={Object.entries(parseProg.parsers || {}).map(([key, v]) => ({ key, name: v.name || key }))}
              states={parseProg.parsers || {}}
              plateforme={parseProg.plateforme}
              masques={parseProg.masques}
              raison={parseProg.raison}
              octetsDistincts={parseProg.octetsDistincts}
              live={parseProg.live}
            />
          )}
          {parseProg?.active && !showImportPanel && (() => {
            const preuve = evidence.find(ev => ev.file_path && ev.file_path === parseProg.collection_dir);
            const nom = preuve?.original_filename || preuve?.name;
            return (
              <ParseTerminal
                caseId={id}
                socket={socket}
                preuves={parseProg.collection_dir ? [parseProg.collection_dir] : null}
                provenance={nom ? [{ nom, sha256: preuve.hash_sha256 || null }] : []}
                titre={t('collection.import.journal.titre', { preuve: nom || t('collection.import.journal.titre_affaire') })}
              />
            );
          })()}
              ...
              <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 16, height: '100%', overflowY: 'auto' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '8px 14px' }}>
                  {[
                    ...
                    <div key={l} style={{ display: 'contents' }}>
                      <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)', whiteSpace: 'nowrap' }}>{l}</span>
                      <span style={{ fontSize: FS_TEXTE, color: 'var(--fl-text)', fontFamily: 'var(--f-mono, monospace)', wordBreak: 'break-word' }}>{v}</span>
                    </div>
                  ))}
                </div>
                <div>
                  <div style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)', marginBottom: 6 }}>{t('casedetail.hashes')}</div>
                  <div style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)', marginBottom: 6 }}>
                    {t('casedetail.hashes_of_archive')}
                    {octetsArchiveDe(drawerEv.metadata) !== null && ` · ${fmtSize(octetsArchiveDe(drawerEv.metadata))}`}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {[['MD5', drawerEv.hash_md5], ['SHA-1', drawerEv.hash_sha1], ['SHA-256', drawerEv.hash_sha256]].filter(([, v]) => v).map(([l, v]) => (
                      <div key={l} onClick={() => navigator.clipboard?.writeText(v)} title={t('common.copy')}
                        ...
                        <div style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-muted)', marginBottom: 2 }}>{l}</div>
                        <div style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, monospace)', color: 'var(--fl-dim)', wordBreak: 'break-all' }}>{v}</div>
                      </div>
                    ))}
                    {![drawerEv.hash_md5, drawerEv.hash_sha1, drawerEv.hash_sha256].some(Boolean) && (
                      <span style={{ fontSize: FS_TEXTE, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, monospace)' }}>{t('casedetail.no_hashes')}</span>
                    )}
                  </div>
                </div>
                {drawerEv.notes && (
                  <p style={{ fontSize: FS_TEXTE, fontStyle: 'italic', color: 'var(--fl-dim)', padding: '6px 8px', background: 'var(--fl-card)', borderRadius: 3, margin: 0 }}>{drawerEv.notes}</p>
                )}
                {drawerEv.additional_files?.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {drawerEv.additional_files.map(f => (
                      <span key={f.name} style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, monospace)', background: 'color-mix(in srgb, var(--fl-accent) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--fl-accent) 18%, transparent)', borderRadius: 3, padding: '1px 6px', color: 'var(--fl-dim)' }}>
                ...
                {(drawerEv.scan_status === 'alert' || drawerEv.scan_status === 'quarantined' || drawerEv.is_suspicious === true) ? (
                  ...
                  <div style={{ borderRadius: 3, padding: 12, textAlign: 'center', background: 'color-mix(in srgb, var(--fl-ok) 7%, transparent)', border: '1px solid color-mix(in srgb, var(--fl-ok) 22%, transparent)' }}>
                    <div style={{ color: 'var(--fl-ok)', fontSize: FS_TEXTE }}>{t('casedetail.clean_file')}</div>
                    <div style={{ color: 'var(--fl-muted)', fontSize: FS_TEXTE, marginTop: 4 }}>{t('casedetail.clean_file_sub')}</div>
                  </div>
                )}
                <button
                  onClick={() => { const tid = drawerEv.id; setDrawerEv(null); navigate(`/cases/${id}/collections/${tid}`); }}
                  style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 12px', borderRadius: 3, cursor: 'pointer', background: 'var(--fl-accent)', color: '#fff', border: 'none', fontFamily: 'var(--f-mono, monospace)', fontSize: FS_TEXTE, fontWeight: 600 }}>
          ...
          <div className="flex justify-between items-center mb-4">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}>
                {t('casedetail.evidence_header')}
              </span>
              <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-muted)' }}>
                {evidence.length}
              </span>
            </div>
          ...
          {showImportPanel && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}>{t('casedetail.import_forensic')}</span>
                <span style={{ fontSize: FS_MICRO, color: 'var(--fl-muted)' }}>Windows: Magnet RESPONSE · KAPE · Velociraptor · CyLR — Hayabusa &nbsp;|&nbsp; Linux: CatScale</span>
              </div>
              <CollectionImportPanel
                caseId={id}
                caseObj={caseData}
                socketAffaire={socket}
                onDone={() => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {evidence.map((ev, _evIdx) => {
              ...
              const declare = evResult?.recordCount ?? 0;
              const recordCount = Number(ev.timeline_rows ?? 0);
              const faits = faitsDePiece({
                type: ev.evidence_type,
                octets: ev.file_size,
                octetsArchive: octetsArchiveDe(ev.metadata),
                verseLe: ev.created_at,
                scanStatus: ev.scan_status,
                declare,
                indexe: recordCount,
              });
              const actions = actionsDePiece({ declare, indexe: recordCount });
              const isAnalyzing = actions.parsing === 'parse' && Boolean(parseProg?.active);
              ...
              return (
                <div key={ev.id} style={{
                  ...
                  <div className="fl-piece"
                    onClick={() => { if (isExpanded) navigate(`/cases/${id}`); else navigate(`/cases/${id}/collections/${ev.id}`); }}>
                    <ChevronRight size={11} className={isExpanded ? 'fl-piece-chev fl-ouvert' : 'fl-piece-chev'} />
                    <div className="fl-piece-ident">
                      <span className="fl-piece-nom" title={ev.name}>{ev.name}</span>
                      <div className="fl-piece-faits">
                        {faits.map((seg, idx) => (
                          <Fragment key={seg.cle}>
                            {idx > 0 && <span className="fl-piece-sep">·</span>}
                            <span
                              className={TON_DU_FAIT[seg.ton] || undefined}
                              title={seg.cle === 'verse' ? t('casedetail.verse_title')
                                : seg.cle === 'archive' ? t('casedetail.archive_title')
                                : seg.cle === 'extrait' ? t('casedetail.extrait_title')
                                : seg.cle === 'ecart' ? t('casedetail.rows_gap_title', { declare: declare.toLocaleString(i18n.language), stocke: recordCount.toLocaleString(i18n.language) })
                                : undefined}>
                              {texteDuFait(seg, t, i18n.language, isAnalyzing)}
                            </span>
                          </Fragment>
                        ))}
                      </div>
                      {ev.additional_files?.length > 0 && (
                        <div className="fl-piece-jointes">
                          {ev.additional_files.map(f => (
                            <span key={f.name} className="fl-piece-jointe">
                              <Paperclip size={9} style={INLINE_PICTO} /> {f.original_name} · {fmtSize(f.size)}
                            </span>
                          ))}
                        </div>
                      )}
                    ...
                    <div className="fl-piece-actions" onClick={e => e.stopPropagation()}>
                      ...
                      <button style={controlStyle(false)} {...controlHover(false)}
                        onClick={() => collectionAPI.parse(id, { evidence_id: ev.id, socketId }).catch(() => {})}
                        title={actions.parsing === 'parse' ? t('casedetail.parse_action_title') : t('casedetail.reparse_title')}>
                        {actions.parsing === 'parse' ? t('casedetail.parse_action') : t('casedetail.reparse')}
                      </button>
                      ...
                      <button style={controlStyle(false)} {...controlHover(false)}
                        disabled={ps.loading}
                        onClick={() => document.getElementById(`pcap-input-${ev.id}`).click()}
                        ...
                        {ps.loading ? t('casedetail.pcap_loading')
                          : ps.result ? t('casedetail.pcap_done', { count: ps.result.inserted })
                          : t('casedetail.add_pcap')}
                      </button>

                      {actions.timeline && (
                        <button style={{ ...controlStyle(false), ...ACTION_SORTIE }} {...SORTIE_SURVOL}
                      ...
                      <button style={controlStyle(false)} {...controlHover(false)}
                        className="fl-piece-marque" data-marquee={ev.is_highlighted ? '1' : '0'}
                        onClick={() => toggleHL(ev.id)}
                        title={t('casedetail.mark_title')}>
                        <Star size={12} fill={ev.is_highlighted ? 'var(--fl-gold)' : 'none'} />
                      </button>

                      <button style={controlStyle(false)} {...controlHover(false)}
                        className="fl-piece-retrait"
                        onClick={() => setEvToDelete(ev)}
                        title={t('casedetail.remove_title')}>
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </div>
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 3, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)',
                        ...
                        {showBar && (
                          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, minWidth: 60, maxWidth: 220 }}>
                            ...
                            <span style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: pct === 100 ? 'var(--fl-ok)' : 'var(--fl-dim)' }}>
            ...
            return (
              <div style={{ marginTop: 20, background: 'var(--fl-panel)', border: '1px solid var(--fl-border)', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderBottom: '1px solid var(--fl-border2)' }}>
                  ...
                  <span style={{ flex: 1 }} />
                  <span style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-subtle)' }}>
                ...
                <div style={{ padding: '14px 16px' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: 16, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>
                    {[
                      ...
                      <div key={label} style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                        ...
                        <span style={{ fontSize: FS_TITRE, fontWeight: 700, color, fontFeatureSettings: '"tnum"' }}>{value}</span>
                        <span style={{ fontSize: FS_MICRO, color: 'var(--fl-muted)' }}>{label}</span>
                      </div>
                    ))}
                  </div>
                  {parsedEvNames.length > 0 && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ fontSize: FS_MICRO, color: 'var(--fl-subtle)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', marginBottom: 6 }}>{t('casedetail.included_collections')}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                        {parsedEvNames.map(name => (
                          <span key={name} style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '2px 7px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-ok) 7%, transparent)', color: 'var(--fl-ok)', border: '1px solid color-mix(in srgb, var(--fl-ok) 15%, transparent)' }}>
                            <Check size={10} style={INLINE_PICTO} /> {name}
                          </span>
                        ))}
                        {evidence.filter(ev => !parsedEvNames.includes(ev.name)).map(ev => (
                          <span key={ev.id} style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '2px 7px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-border) 9%, transparent)', color: 'var(--fl-muted)', border: '1px solid var(--fl-border)' }}>
                            ○ {ev.name}
                          </span>
                        ))}
                      </div>
                  ...
                  {parsedCount === 0 && (
                    <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-dim)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '0 0 10px', textAlign: 'center', fontStyle: 'italic' }}>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: selectedTemplate ? 'var(--fl-dim)' : 'var(--fl-muted)', flex: 1 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 10px', borderRadius: 3, fontSize: FS_TEXTE,
                ...
                {showComposer && !selectedTemplate && (
                  <div style={{ marginBottom: 12, padding: '12px 14px', borderRadius: 3, background: 'var(--fl-bg)', border: '1px solid var(--fl-border2)' }}>
                    <div style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-subtle)', marginBottom: 8 }}>{t('casedetail.sections_to_include')}</div>
                      ...
                      {[['mitre', 'MITRE ATT&CK'], ['killchain', t('casedetail.report_section_killchain')], ['findings', t('casedetail.report_section_detections')], ['iocs', 'IOCs'], ['timeline', t('casedetail.report_section_timeline')], ['evidence', t('casedetail.report_section_evidence')]].map(([key, label]) => {
                        ...
                        return (
                          <button key={key}
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 3, cursor: 'pointer', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)',
                    ...
                    <div style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-subtle)', marginTop: 6 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                      <button onClick={() => setAiEnabled(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                        ...
                        <span style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: aiEnabled ? 'var(--fl-text)' : 'var(--fl-muted)' }}>{t('casedetail.enrich_with_ai')}</span>
                      </button>
                      {aiEnabled && (
                        <span style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-subtle)' }}>
                    ...
                    <ReportWritingPane
                      caseId={id}
                      doc={reportDoc}
                      aiEnabled={aiEnabled && parsedCount > 0}
                      aiLoading={aiLoading}
                      aiError={aiError}
                      onPrefill={generateAiDraft}
                      conflits={conflitsIa}
                      onRemplacer={remplacerConflitsIa}
                      onIgnorer={garderConflitsIa}
                    />
                  </div>
                )}
                {showComposer && selectedTemplate && (
                  <div style={{ marginBottom: 12, padding: '10px 12px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-warn) 6%, transparent)', border: '1px solid color-mix(in srgb, var(--fl-warn) 20%, transparent)', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-warn)' }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 14 }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '8px 16px', borderRadius: 3, fontSize: FS_TEXTE,
                  ...
                  {reportDone && (
                    <button
                      ...
                      style={{
                        display: 'flex', alignItems: 'center', gap: 6,
                        padding: '8px 14px', borderRadius: 3, fontSize: FS_TEXTE,
                ...
                {reportError && <Alert message={reportError} style={{ marginTop: 10 }} />}
                {reportDone && (
                  <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 3, background: 'color-mix(in srgb, var(--fl-ok) 3%, transparent)', border: '1px solid color-mix(in srgb, var(--fl-ok) 15%, transparent)', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-ok)', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {evToDelete && (
        <SuppressionPreuve
          evidence={evToDelete}
          onClose={() => setEvToDelete(null)}
          onDeleted={supprimee => {
            setEvidence(prev => prev.filter(e => e.id !== supprimee));
            if (selEv?.id === supprimee) setSelEv(null);
            setEvToDelete(null);
          }}
        />
      )}
      ...
      >
        <Modal.Body>
          <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_MICRO, color: 'var(--fl-danger)', marginBottom: 14, letterSpacing: '0.04em' }}>
            {hardDeleteResult ? t('casedetail.db_verification') : t('casedetail.admin_irreversible_action')}
          </div>

          {!hardDeleting && !hardDeleteResult && (
            <>
              <div style={{ marginBottom: 16, padding: '10px 14px', borderRadius: 3, background: 'rgba(218,54,51,0.06)', border: '1px solid rgba(218,54,51,0.18)', fontSize: FS_TEXTE, color: 'var(--fl-muted)', lineHeight: 1.7 }}>
                ...
                <span style={{ color: 'var(--fl-warn)', fontSize: FS_TEXTE }}>{t('casedetail.audit_record_kept')}</span>
              </div>
              <div style={{ marginBottom: 6, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}>
          ...
          {hardDeleting && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '32px 0', gap: 14 }}>
              ...
              <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE, color: 'var(--fl-muted)' }}>{t('casedetail.secure_delete_running')}</div>
              <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-dim)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>DoD 5220.22-M · cascade delete · audit log</div>
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE, fontWeight: 700, color: hardDeleteResult.ok ? 'var(--fl-ok)' : 'var(--fl-danger)' }}>
                ...
                {hardDeleteResult.ok && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginLeft: 26 }}>
                    <div style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-ok)' }}>
                    ...
                    <div style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: hardDeleteResult.verified ? 'var(--fl-ok)' : 'var(--fl-danger)' }}>
                    ...
                    {hardDeleteResult.files_errors?.length > 0 && (
                      <div style={{ fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-warn)' }}>
                ...
                {!hardDeleteResult.ok && (
                  <div style={{ marginLeft: 26, fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-danger)' }}>
                    {hardDeleteResult.error}
                  </div>
                )}
              </div>

              <div style={{ padding: '8px 12px', borderRadius: 3, background: 'var(--fl-bg)', border: '1px solid var(--fl-border)', fontSize: FS_TEXTE, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-dim)' }}>
      ...
      >
        <Modal.Body>
          <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-dim)', marginBottom: 16, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>
            {c?.case_number} · {c?.title}
          </div>

          {statusModal === '_pick' ? (
            <>
              <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-muted)', marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {Object.entries(SM).filter(([k]) => k !== c?.status).map(([key, { l, c: col }]) => (
                  <button key={key} onClick={() => setStatusModal(key)}
                    ...
                    <span style={{ fontSize: FS_TEXTE, fontWeight: 600, color: col }}>{l}</span>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <div style={{ padding: '12px 14px', borderRadius: 3, marginBottom: 18,
                ...
                fontSize: FS_TEXTE, color: 'var(--fl-text)', lineHeight: 1.6,
              }}>
                {statusModal === 'closed' ? (
                  <>
                    ...
                    <span style={{ fontSize: FS_TEXTE, color: 'var(--fl-dim)' }}>{t('casedetail.status_reversible_hint')}</span>
                  </>
                ) : statusModal === 'pending' ? (
              ...
              <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-muted)', marginBottom: 4 }}>
        ...
        <Modal.Body>
          {triageRunning && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '32px 0', gap: 14 }}>
              ...
              <div style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE, color: 'var(--fl-dim)' }}>
                {t('casedetail.triage_running')}
              </div>
              <div style={{ fontSize: FS_TEXTE, color: 'var(--fl-muted)', fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)' }}>
            ...
            <div>
              {triageData.case_indicators && (
                <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                  {[
                    ...
                    <span key={label} style={{ fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '3px 10px',
              ...
              {triageData.scores?.length === 0 && (
                <div style={{ padding: '24px', textAlign: 'center', color: 'var(--fl-muted)',
                  fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE, borderRadius: 3,
                    ...
                    return (
                      <div key={m.hostname} style={{ padding: '10px 14px', borderBottom: '1px solid #1c2a3a' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                          <span style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TEXTE, color: 'var(--fl-text)', flex: 1 }}>{m.hostname}</span>
                          <span style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_MICRO, color: 'var(--fl-muted)' }}>{m.event_count?.toLocaleString()} {t('casedetail.event_abbr')}</span>
                          <span style={{ fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', fontSize: FS_TITRE, fontWeight: 700, color, width: 30, textAlign: 'right' }}>{m.score}</span>
                          <span style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '1px 6px', borderRadius: 3,
                        ...
                        {Object.keys(breakdown).length > 0 && (
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {Object.entries(breakdown).map(([rule, pts]) => (
                              <span key={rule} style={{ fontSize: FS_MINI, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', padding: '1px 5px',
              ...
              {triageData.computed_at && (
                <div style={{ marginTop: 10, fontSize: FS_MICRO, fontFamily: 'var(--f-mono, "JetBrains Mono", monospace)', color: 'var(--fl-muted)', textAlign: 'right' }}>
      ...
      >
        <Modal.Body>
          <div style={{ marginBottom: 14, padding: '10px 14px', borderRadius: 3,
            ...
            fontSize: FS_TEXTE, color: 'var(--fl-muted)', lineHeight: 1.7 }}>
      ...
      >
        <Modal.Body>
          <div style={{ marginBottom: 14, padding: '10px 14px', borderRadius: 3,
            ...
            <span style={{ fontSize: FS_TEXTE, color: 'var(--fl-dim)' }}>
